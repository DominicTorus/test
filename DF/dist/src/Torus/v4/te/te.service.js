...
let TeService = TeService_1 = class TeService {
    constructor(poClient, redisService, securityService, jwtService, schedulerRegistry, CommonService, lockservice, processor) {
    ...
    async addEventEmitterJob(queueName, pfdto, jobName, processedKey, pfs, currentFabric, options) {
        try {
            ...
            if (pfdto && (Object.keys(pfdto.data).length > 0 || pfdto.data.length > 0)) {
                ...
                await Promise.all(preprocessingTasks.map(({ upId }) => this.pfPreProcessor(processedKey, pfs, upId, currentFabric)));
        ...
        for (var j = 0; j < poNode.length; j++) {
                ...
                try {
                    ...
                    if (customConfig) {
                        ...
                        if (oprname && oprkey.includes(oprname)) {
                            ...
                            else if (oprname == 'select' && (!dbres || dbres?.length == 0)) {
            ...
            if (nodeType == 'filenode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    if (customConfig) {
                        ...
                        if (!childInsertArr || childInsertArr?.length == 0 && !textobj)
                ...
                try {
                    ...
                    if (customConfig) {
                        ...
                        if (currentFabric == 'PF-SCDL') {
                            ...
                            let childJobName = `${((pfdKeyParts[1] + pfdKeyParts[5] + pfdKeyParts[7] + pfdKeyParts[9] + pfdKeyParts[11] + pfdKeyParts[13]).replace(/[-_]/g, '')).replace(/\s+/g, '')}-${upId}`;
                            this.logger.log(`Creating child job in queue: ${childJobName}`);
                            this.logger.log(`Parent job: ${parentJobName}, Parent job ID: ${upId}`);
                            const parentQueue = this.getQueue(parentJobName);
                            const childQueue = this.getQueue(childJobName);
                ...
                try {
                    ...
                    if (customConfig) {
                        ...
                        if (internalEdges && internalEdges.hasOwnProperty(poNode[j].nodeId)) {
                            ...
                            let sourcekey = [];
                            let rootarr = [];
                            let statickeyword = ["get", "post", "200", '201', '202', '204', '400', '401', '403', '404', '500', "parameters", "requestBody", "responses", "content", "application/json", "application/xml", "schema", "properties", "allOf", "oneOf", "inputschema", "outputschema"];
                            let numberArr = Array.from({ length: 101 }, (_, i) => (i).toString());
                            ...
                            for (let j = 0; j < edgesarr.length; j++) {
                                ...
                                if (targetHandle.includes('.')) {
                                    ...
                                    let staticRemove = targetVaribale.filter((item) => !statickeyword.includes(item));
                ...
                try {
                    ...
                    if (customConfig) {
                        ...
                        if (DfdFabric == 'DF-DFD') {
                            ...
                            var RCMresult = await this.CommonService.getRuleCodeMapper(poNode[j], DfExecutedDataSet, processedKey, currentFabric, SessionInfo);
                            let zenresult = RCMresult.rule;
                            let customcoderesult = RCMresult.code;
                            let response;
                            if (customcoderesult != undefined) {
            ...
            if (nodeType == 'xlsx2jsonconverternode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    if (Array.isArray(inputparam) && inputparam?.length > 0) {
                        for (let i = 0; i < inputparam.length; i++) {
                            inputparam[i] = Object.assign(inputparam[i], { [nodeName]: childInsertArr });
                        }
                    }
                    else if (typeof inputparam == 'object') {
                        inputparam = Object.assign(inputparam, { [nodeName]: childInsertArr });
                    }
    ...
    async DFDMapEdgeValues(poNode, currentNodeEdge, inputparam, processedKey, upId, collectionName, statickeyword, numberArr, parameter, codeObj, pfo, fabric) {
        var mapObj = {};
        var textobj;
        var tempQryVal = [];
        let afp = {};
    ...
    async mapEdgeValuesToParams(pfdto, currentNodeEdge, inputparam, processedKey, upId, collectionName, statickeyword, numberArr, parameter, codeObj, pfo, childtable) {
        try {
            let childInsertArr = [];
            let srcIdArr = [];
            let mapObj, tempQryVal, targetVal, staticRemove, textobj;
    ...
    async setMapobj(mapObj, sourceStatus, upId, tokenDecode, emaildecode, processedKey, nodename, apiUrl, requestConfig, collectionName, method, encCredentials) {
        if (!mapObj)
            throw new customException_1.CustomException(`Mapping was required in ${nodename}`, 400);
        mapObj['trs_status'] = sourceStatus;
        mapObj['trs_process_id'] = upId;
...
exports.TeService = TeService = TeService_1 = __decorate([
    ...
    __param(7, (0, common_1.Inject)((0, common_1.forwardRef)(() => event_emitter_processor_1.EventEmitterProcessor))),
