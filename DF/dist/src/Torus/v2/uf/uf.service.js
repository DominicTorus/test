...
const fusionAuth_api_1 = require("../../../fusionAuth.api");
...
let UfService = class UfService {
    constructor(jwtService, jwt, gorule, redisService, commonService, mongoService) {
    ...
    async screenRoute(keys, token, header) {
        try {
            for (let i = 0; i < keys.length; i++) {
                const UO = await this.commonService.readAPI(keys[i].ufKey + ':UO', process.env.CLIENTCODE, token);
                const securityData = UO.securityData;
                const screenName = keys[i].ufKey.split(':')[11];
                let templateArray = securityData.accessProfile;
                ...
                if (keys[i].ufKey === securityData.afk) {
                    for (let j = 0; j < templateArray.length; j++) {
                        if (accessProfile.accessProfile.includes(templateArray[j].accessProfile) &&
    ...
    async setUpKey(key, token) {
        try {
            const sKey = await this.commonService.readAPI(key, process.env.CLIENTCODE, token);
            if (sKey) {
                if (sKey?.tenantAppearancekey) {
                    const presetData = await this.commonService.readAPI(sKey?.tenantAppearancekey, process.env.CLIENTCODE, token);
    ...
    async getpagination(key, page, count, filter, searchObj, token) {
        try {
                ...
                if (filter) {
                    ...
                    if (rule?.nodes?.length && rule?.edges?.length) {
                        for (let j = 0; j < data.length; j++) {
                            let result = await this.gorule.goRule(rule, data[j]);
                            if (result?.error) {
                                break;
                            }
                        ...
                        if (searchObj && Object.keys(searchObj).length > 0) {
                            for (var x = 0; x < finalData.length; x++) {
                                ...
                                if (s == searchkey.length)
                                    searcharr.push(finalData[x]);
                            }
    ...
    async Orchestration(key, componentId, controlId, token, isTable, accessProfile) {
        try {
            const UO = await this.commonService.readAPI(key + ':UO', process.env.CLIENTCODE, token);
            const screenName = key.split(':')[11];
            let mappedData = UO.mappedData.artifact.node;
            const securityData = UO.securityData;
            let templateArray = securityData.accessProfile;
            ...
            if (UO) {
                if (key && !componentId && !controlId) {
                    if (key === securityData.afk) {
                        for (let i = 0; i < templateArray.length; i++) {
                            if (accessProfile.includes(templateArray[i].accessProfile) &&
                                ...
                                templateArray[i].security.artifact?.node?.map((nodes) => {
                                    allowedGroup.push({
                                ...
                                if (accessProfile.includes(templateArray[i].accessProfile)) {
                                    if (screenName === templateArray[i].security.artifact.resource &&
                                        ...
                                        for (let l = 0; l < templateArray[i].security.artifact.node.length; l++) {
                                            selectedValues.push(templateArray[i].security.artifact.node[l].SIFlag
                                                .selectedValue);
                                        }
                                        for (let m = 0; m < templateArray[i].security.artifact.node.length; m++) {
                                            if (selectedValues.includes('ATO')) {
                                                if (templateArray[i].security.artifact.node[m].SIFlag
                                                    .selectedValue === 'ATO') {
                                                    componentNameArray.push(templateArray[i].security.artifact.node[m].resource.toLowerCase());
                                                    break;
                                                }
                                            }
                                            else {
                                            ...
                                            if (isTable === true) {
                                                for (let k = 0; k <
                                                    templateArray[i].security.artifact.node[j].objElements
                                                        .length; k++) {
                                                    if (templateArray[i].security.artifact.node[j]
                                                        .objElements[k].SIFlag.selectedValue !== 'BTO') {
                                                        controlNames.push(templateArray[i].security.artifact.node[j]
                                                            .objElements[k].resource);
                                                    }
                                                    if (templateArray[i].security.artifact.node[j]
                                                        .objElements[k].SIFlag.selectedValue == 'RTO') {
                                                        readableControls.push(templateArray[i].security.artifact.node[j]
                                                            .objElements[k].resource);
                                                    }
                                                }
                                                controlNames = controlNames.map((item) => item.toLowerCase());
                                                readableControls = readableControls.map((item) => item.toLowerCase());
                                            }
                                            else {
                                                for (let k = 0; k <
                                                    templateArray[i].security.artifact.node[j].objElements
                                        ...
                                        if (templateArray[i].security.artifact.node[j].SIFlag
                                            .selectedValue === 'AA' ||
                                            templateArray[i].security.artifact.node[j].SIFlag
                                                .selectedValue === 'RA') {
                                            if (isTable === true) {
                                        ...
                                        else if (templateArray[i].security.artifact.node[j].SIFlag
                                            .selectedValue === 'BTO' ||
                                            templateArray[i].security.artifact.node[j].SIFlag
                                                .selectedValue === 'BA') {
                                            controlNames = controlNames.map((item) => item.toLowerCase());
                        ...
                        if (componentId === mappedData[i].nodeId) {
                            for (let j = 0; j < mappedData[i].objElements.length; j++) {
                                ...
                                if (componentId == mappedData[i].objElements[j]?.parentId) {
                                    let tempRule = mappedData[i].objElements[j]?.rule;
                    ...
                    if (mappedData) {
                        for (let i = 0; i < mappedData.length; i++) {
                            if (componentId === mappedData[i].nodeId) {
                                for (let node = 0; node < mappedData[i].objElements.length; node++) {
                                    if (mappedData[i].objElements[node].mapper.length > 0) {
                                        mappperNodeId = mappedData[i].objElements[node].mapper[0].sourceKey[0].split('|')[1];
                                        dfKey =
                                            mappedData[i].objElements[node].mapper[0].sourceKey[0].split('|')[0];
                                        dfKey = dfKey + ':';
                                    }
                                }
                            }
                    ...
                    if (isTable) {
                        let dfSchemaKey = await this.commonService.readAPI(dfKey + 'DFO', process.env.CLIENTCODE, token);
                        try {
                            dfData = dfSchemaKey;
                            let schemaData = dfData.filter((item) => {
                                if (item?.nodeType !== 'startnode' &&
                                    item?.nodeType !== 'endnode')
                                    return item;
                            });
                            let nodeType = 'apinode';
                            schemaData.map((nodes) => {
                            ...
                            for (let j = 0; j < mappedData[i].objElements.length; j++) {
                                if (controlId === mappedData[i].objElements[j].elementId) {
                                    if (mappedData[i].objElements[j].mapper.length == 0) {
                                        ...
                                        let dfdKey = mappedData[i].objElements[j].mapper[0].sourceKey[0].split('|')[0];
                                        let dfSchemaKey = await this.commonService.readAPI(dfdKey + ':DFO', process.env.CLIENTCODE, token);
                                        try {
                                            dfData = dfSchemaKey;
                                            schemaData = dfData.filter((item) => {
                                                if (item?.nodeType == 'apinode' ||
                                    ...
                                    if (mappedData[i].objElements[j]?.elementType == "dynamicjsonform") {
                                        let ruleKey = '';
                                        let pfRuleData = {};
                                        let NDPData = await this.commonService.readAPI(key + ':NDP', process.env.CLIENTCODE, token);
                                        if (controlId in NDPData) {
                                            ruleKey = NDPData[controlId]?.apiKey || '';
                                            let temp = await this.commonService.readAPI(ruleKey, process.env.CLIENTCODE, token);
                                    ...
                                    if (mappedData[i].objElements[j]?.elementType == "editor") {
                                        let editorMapper = [];
                                        object?.mapper?.map((eachResource) => {
                                            let temp = {
    ...
    async elementsFilter(key, groupName, controlName, token) {
        try {
            let rule = '';
            const uoKey = await this.commonService.readAPI(key + ':UO', process.env.CLIENTCODE, token);
            let UO = uoKey;
            let elements = {};
            let artifact = key.split(':')[11];
            elements = {};
    ...
    async getMapperDetails(key, componentId, controlId, category, bindtranValue, code, token) {
        try {
            let codName;
            const uoKey = await this.commonService.readAPI(key + ':UO', process.env.CLIENTCODE, token);
            let UO = uoKey;
            if (UO) {
                ...
                if (mappedData) {
                    if (key && !componentId && !controlId) {
                        if (UO.mappedData.artifact.mapper.length == 0)
                            return [];
                        ...
                        for (let i = 0; i < mappedData.length; i++) {
                            if (componentId === mappedData[i].nodeId) {
                                if (mappedData[i].mapper.length == 0)
                                    return [];
                                ...
                                for (let j = 0; j < mappedData[i].objElements.length; j++) {
                                    if (controlId === mappedData[i].objElements[j].elementId) {
                                        if (mappedData[i].objElements[j].mapper.length == 0)
                                            return [];
                                        let dfdKey = mappedData[i].objElements[j].mapper[0].sourceKey[0].split('|')[0];
                                        let mapperColumn = mappedData[i].objElements[j].mapper[0].sourceKey[0].split('|')[2];
                                        let dstKey = dfdKey
                                            .replace(':AFC:', ':AFCP:')
                                            .replace(':AF:', ':AFP:')
                                            .replace(':DF-DFD:', ':DF-DST:');
                                        let dfData = await this.commonService.readAPI(dstKey + ':DS_Object', process.env.CLIENTCODE, token);
                                        dfData = dfData;
                                        if (key &&
                                            componentId &&
                                            controlId &&
                                            ...
                                            for (let j = 0; j < categoryData.length; j++) {
                                                Object.keys(categoryData[j]).map((keyName) => {
                                                    if (categoryData[j].parentCode === code &&
    ...
    async codeExecution(stringCode, params) {
        try {
    ...
    async codefilter(key, groupId, controlId, event, token) {
        try {
            let rule = '';
            const uoKey = await this.commonService.readAPI(key + ':UO', process.env.CLIENTCODE, token);
            let UO = uoKey;
            ...
            if (mappedData) {
                if (groupId) {
                    if (event) {
                        ...
                        let eventDetails = await this.eventFunction(eventProperty);
                                ...
                                if (group.node == groupId) {
                                    for (let j = 0; j < group.objElements.length; j++) {
                                        ...
                                        if (control.code != '')
                                            ...
                                            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'TG037', 'there is no rule in control level', key, token);
                                    }
                                }
                            }
                        }
                        ...
                        for (let i = 0; i < mappedData.artifact.node.length; i++) {
                            ...
                            if (group.nodeId == groupId) {
                                if (group.code != '')
                                    ...
                                    await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'TG038', 'there is no rule in group level', key, token);
                            }
                        }
                    }
                }
                else {
                    if (mappedData.artifact.code != '')
                        return mappedData.artifact.code;
                    else
                        await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'TG039', 'there is no rule in artifact level', key, token);
                }
            }
        }
        catch (error) {
    ...
    async ifo(formData, key, controlId, isTable, token) {
        if (isTable == true) {
            try {
                ...
                if (key !== '') {
                    let spiltedkey = key.split('|');
                    let findingkey = spiltedkey.pop();
                    let newKey = structuredClone(spiltedkey);
                    const POdataKey = await this.commonService.readAPI(spiltedkey.join(':') + ':PO', process.env.CLIENTCODE, token);
                            ...
                            for (let i = 0; i < POdata.mappedData.artifact.node.length; i++) {
                                if (POdata.mappedData.artifact.node[i].nodeId == findingkey) {
                                    if (POdata.mappedData.artifact.node[i].ifo) {
                                        let filterItems = {};
                                        for (let j = 0; j < POdata.mappedData.artifact.node[i].ifo.length; j++) {
                                            let NodeId = POdata.mappedData.artifact.node[i].ifo[j].nodeId.split('.')[0];
                                            if (NodeId == controlId) {
                                                let nodeName = POdata.mappedData.artifact.node[i].ifo[j].name;
                                ...
                                if (POdata.mappedData.artifact.node[i].nodeId == findingkey) {
                                    if (POdata.mappedData.artifact.node[i].ifo) {
                                        ...
                                        for (let j = 0; j < POdata.mappedData.artifact.node[i].ifo.length; j++) {
                                            let NodeId = POdata.mappedData.artifact.node[i].ifo[j].nodeId.split('.')[0];
                                            if (NodeId == controlId) {
                                                let nodeName = POdata.mappedData.artifact.node[i].ifo[j].name.toLocaleLowerCase();
                                        ...
                                        if ('childTables' in formData) {
                                            formData.childTables.map((eachTable) => {
    ...
    async fetchActionDetails(key, groupId, controlName, token) {
        try {
            const uoKey = await this.commonService.readAPI(key + ':UO', process.env.CLIENTCODE, token);
            let UO = uoKey;
            if (UO) {
    ...
    async fetchRuleDetails(key, groupId, controlId, token) {
        try {
            const uoKey = await this.commonService.readAPI(key + ':UO', process.env.CLIENTCODE, token);
            let UO = uoKey;
            if (UO) {
    ...
    async InitiatePF(key, sourceId, token) {
        try {
            if (key !== '') {
                let spiltedkey = key.split('|')[0];
                let findingkey = key.split('|')[1];
                const NDSdataKey = await this.commonService.readAPI(spiltedkey + ':NDS', process.env.CLIENTCODE, token);
                const NDSdata = NDSdataKey;
                const POdataKey = await this.commonService.readAPI(spiltedkey + ':PO', process.env.CLIENTCODE, token);
                const POdata = POdataKey;
                let nodeProperty = {
                    key: spiltedkey,
                };
                        ...
                        POdata?.mappedData?.artifact?.node.map((nodes) => {
                            if (nodes.nodeId === findingkey && nodes.events.length > 0) {
                                for (let i = 0; i < nodes.events.length; i++) {
                                    if (nodes.events[i].sourceId.replace(/\//g, '|') === sourceId) {
    ...
    async getPFDetails(isTable, key, groupId, controlId, token) {
        try {
            let eventProperty;
            let eventDetails;
                            ...
                            if (isTable) {
                                if (Object.keys(mappedData[i].events).length > 0) {
                                    ...
                                    for (let k = 0; k < eventDetailsArray.length; k++) {
                                        if (eventDetailsArray[k].type === 'handlerNode' &&
                                            eventDetailsArray[k].name === 'saveHandler') {
                                            if (eventDetailsArray[k].targetKey &&
                                        ...
                                        else if (eventDetailsArray[k].type === 'handlerNode' &&
                                ...
                                for (let j = 0; j < mappedData[i].objElements.length; j++) {
                                    if (controlId === mappedData[i].objElements[j].elementId) {
                                        if (Object.keys(mappedData[i].objElements[j].events).length >
                                            0) {
                                            eventProperty =
    ...
    async getDfkey(ufKey, groupid, token) {
        try {
            let sourceData;
            const source = 'redis';
            const target = 'redis';
            let DFkeys = [];
            const mapperPropertiesKey = await this.commonService.readAPI(ufKey + ':UO', process.env.CLIENTCODE, token);
            const mapperProperties = mapperPropertiesKey;
            if (mapperProperties) {
                if (groupid) {
                            ...
                            if (groupid === sourceData[i].nodeId) {
                                ...
                                for (let node = 0; node < sourceData[i].objElements.length; node++) {
                                    if (sourceData[i].objElements[node].mapper.length > 0) {
                                        dfKey =
                                            sourceData[i].objElements[node].mapper[0].sourceKey[0].split('|')[0];
                                        dfKey = dfKey + ':';
                                        return dfKey;
                                    }
                                }
    ...
    async paginationDataFilter(ufKey, data, token, dfdType, primaryKey) {
        try {
            const source = 'redis';
            const target = 'redis';
            const mapperPropertiesKey = await this.commonService.readAPI(ufKey + ':UO', process.env.CLIENTCODE, token);
            ...
            if (mapperProperties) {
                if (data == undefined || data.length == 0)
                    await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'TG062', 'Record not found', ufKey, token);
                if (mapperProperties.mappedData) {
                    let mapperSourceData = {};
                    let mapperData = [];
                    let objectfn = [];
                    let rule = [];
                    let mapperSourceDataKeys = [];
                    ...
                    let IdAndName = [];
                    mapperProperties.mappedData.artifact.node.forEach((element) => {
                        ...
                        element.objElements.forEach((element) => {
                            IdAndName.push({
                    ...
                    for (let i = 0; i < mapperData.length; i++) {
                        ...
                        targetKeys.push({
                            targetKey: mapperData[i].targetKey.split('|')[mapperData[i].targetKey.split('|').length - 1],
                            columnKey: temp,
                        });
                            ...
                            Object.keys(ele).map((key) => {
                                ...
                                for (let i = 0; i < targetKeys.length; i++) {
                                    if (targetKeys[i].targetKey.toLowerCase() === keyName) {
                                        temp = {
                                            ...temp,
                                            [targetKeys[i].targetKey.toLowerCase()]: ele[targetKeys[i].columnKey?.toLowerCase()],
                                        };
                                    }
                                }
                            });
                        ...
                        for (let l = 0; l < objectfn.length; l++) {
                            if (objectfn[l].name != '' && objectfn[l].code != '') {
                                for (let i = 0; i < newData.length; i++) {
                                    const transformFunction = new Function('v', `return ${objectfn[l].code};`)(v);
                                    let result = v.safeParse(transformFunction, newData[i][objectfn[l].name]);
                                    if (result.success) {
                                        newData[i] = {
                                            ...newData[i],
    ...
    async setSaveHandlerData(key, value, path) {
        try {
            ...
            await this.redisService.setJsonData(key, value, process.env.CLIENTCODE, path);
        }
        catch (error) {
            throw new common_1.BadGatewayException(error);
        }
    ...
    async uploadHandlerData(key) {
        try {
            const flag = await this.redisService.getJsonData(key, process.env.CLIENTCODE);
            let value = {
                params: {
                    request: {},
                    response: {},
    ...
    async SFCheckScreen(ufKey, token, nodeId, isTable) {
        try {
            const screenName = ufKey.split(':')[11];
            const source = 'redis';
            const target = 'redis';
            const decodedToken = await this.jwtService.decodeToken(token);
            const DOKey = await this.commonService.readAPI(ufKey + ':UO', process.env.CLIENTCODE, token);
            const DO = DOKey;
            if (DO) {
                const securityData = DO.securityData;
                const templateArray = securityData.accessProfile;
                if (ufKey === securityData.afk) {
                    if (!nodeId) {
                        for (let i = 0; i < templateArray.length; i++) {
                            if (decodedToken.accessProfile.includes(templateArray[i].accessProfile) &&
                                screenName === templateArray[i].security.artifact.resource) {
                                return {
                                    result: templateArray[i].security.artifact.SIFlag.selectedValue,
                                };
                            }
                        }
                    }
                                                    ...
                                                    for (let j = 0; j < templateArray[i].security.artifact.node.length; j++) {
                                                        if (nodeId ===
                                                            templateArray[i].security.artifact.node[j]
                                                                .resourceId) {
                                                            componentNameArray.push(templateArray[i].security.artifact.node[j].resource.toLowerCase());
                                                        }
                                                    }
    ...
    async logout(headers, tokens, key) {
        try {
            ...
            const sessionListCache = await this.redisService.getJsonData(sessionListCacheKey, process.env.CLIENTCODE);
            ...
            if (updatedSessionList?.find((item) => item?.sid == payload.sid)) {
                await this.redisService.setJsonData(sessionListCacheKey, JSON.stringify(updatedSessionList.filter((s) => s?.sid !== payload.sid)), process.env.CLIENTCODE);
            }
            else {
                await this.redisService.setJsonData(sessionListCacheKey, JSON.stringify(updatedSessionList), process.env.CLIENTCODE);
            }
    ...
    async getAccessToken(token, selectedCombination, selectedAccessProfile, dap, ufClientType) {
        try {
            const accessProfileCacheKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:securityTemplate`;
            const accessProfileCache = await this.redisService.getJsonData(accessProfileCacheKey, process.env.CLIENTCODE);
            const accessProfileList = accessProfileCache
                ? JSON.parse(accessProfileCache)
                : [];
            const filteredAccessprofile = accessProfileList.find((t) => t?.accessProfile === selectedAccessProfile);
            const filteredCombination = this.transformToCombinations([
                filteredAccessprofile,
            ]);
            const accessObj = {
                ...filteredCombination[0]?.combinations?.find((c) => {
                    return (c?.orgGrpCode === selectedCombination?.orgGrpCode &&
            ...
            const sessionListResponse = await this.redisService.getJsonData(sessionListCacheKey, process.env.CLIENTCODE);
            ...
            if (sessionListResponse) {
                ...
                const previousActiveSession = updatedSessionList.find((session) => session?.sid === sid);
                updatedSessionList = updatedSessionList.filter((s) => s?.sid !== sid).concat({
            ...
            await this.redisService.setJsonData(sessionListCacheKey, JSON.stringify(updatedSessionList), process.env.CLIENTCODE);
            await this.redisService.setJsonData(`CK:${tenant}:FNGK:AFP:FNK:${ufClientType}:CATK:${ag}:AFGK:${app}:AFK:session:AFVK:${version}:${loginId}_variables`, JSON.stringify(accessObj), process.env.CLIENTCODE);
            return updatedToken;
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH006', error, 'getAccessToken', token, {
                artifact: 'getAccessToken',
            });
            throw new common_1.BadGatewayException(error);
        }
    }
    transformToCombinations(data) {
        try {
            return data.map((profile) => {
                const combinations = [];
                profile.orgGrp?.forEach((orgGrp) => {
                    ...
                    orgGrp.org?.forEach((org) => {
                        ...
                        org.subOrgGrp?.forEach((subOrgGrp) => {
                            const { subOrgGrpCode, subOrgGrpName } = subOrgGrp;
                            subOrgGrp.subOrg?.forEach((subOrg) => {
                                const { subOrgCode, subOrgName } = subOrg;
                                subOrg.psGrp?.forEach((psGrp) => {
                                    const { psGrpCode, psGrpName } = psGrp;
                                    psGrp.ps?.forEach((ps) => {
                                        const { psCode, psName } = ps;
                                        ps.roleGrp?.forEach((roleGrp) => {
    ...
    async getAccessTemplate(token) {
        try {
            ...
            const accessTemplateResponse = await this.redisService.getJsonData(accessTemplateCacheKey, process.env.CLIENTCODE);
            if (accessTemplateResponse) {
                const accessTemplate = JSON.parse(accessTemplateResponse);
                const filteredAccessTemplate = accessTemplate.filter((template) => accessProfile.includes(template?.accessProfile));
                return this.transformToCombinations(filteredAccessTemplate);
            }
            else {
                throw new customException_1.NotFoundException('Access Template not found');
            }
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH007', error, 'select-context', token, {
                artifact: 'select-context',
            });
            throw new common_1.BadGatewayException(error);
        }
    }
    async fusionAuthVerifyRefreshToken(refreshToken) {
        try {
            ...
            if (!res.ok) {
                throw new customException_1.UnauthorizedException('Invalid access token');
            }
    ...
    async checkSession(sessionList) {
        try {
            ...
            for (let index = 0; index < sessionList.length; index++) {
                ...
                const sessionLastUpdatedTime = new Date(session['updatedOn'] || session['createdOn']).getTime() /
                    1000;
    ...
    async MyAccountForClient(token, key, authorization) {
        if (authorization) {
            try {
    ...
    async introspectToken(headers, key, tokens) {
        try {
            const { authorization } = headers;
            if (!authorization || typeof authorization !== 'string') {
            ...
            const updatedSessionList = await this.checkSession(sessionList);
            let currentSession = updatedSessionList.find((item) => item?.sid == payload.sid);
            if (!currentSession) {
                await this.redisService.setJsonData(sessionListCacheKey, JSON.stringify(updatedSessionList), process.env.CLIENTCODE);
                await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'TG075', 'Invalid access token', key, tokens);
                throw new customException_1.UnauthorizedException('Invalid access token');
            ...
            if (!refreshToken) {
                await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'TG075', 'Session not available', key, tokens);
                throw new customException_1.UnauthorizedException('Session not available');
            }
            if (currentSession['refreshTokenId']) {
                ...
                try {
                    await this.jwt.verifyAsync(currentSession['refreshToken'], {
                        secret: auth_secret
                    });
                }
                catch (error) {
            ...
            await this.redisService.setJsonData(sessionListCacheKey, JSON.stringify(updatedSessionList
                .filter((s) => s.sid !== payload.sid)
                .concat(currentSession)), process.env.CLIENTCODE);
    ...
    isUserAccessExpired(user) {
        if (!('accessProfile' in user)) {
            throw new customException_1.NotAcceptableException('Access profile not found, Please contact administrator');
        }
    ...
    async signIntoTorus(username, password, ufClientType, isOauthUser = false, fusionAuthLoginResponse) {
        try {
            let tenantUserKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:TENANT:AFGK:${tenant}:AFK:PROFILE:AFVK:v1:users`;
            const appUserKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:users`;
            const sessionListCacheKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:session`;
            let tenantUser = JSON.parse(await this.redisService.getJsonData(tenantUserKey, process.env.CLIENTCODE));
            let appUser = [];
            if (tenantUser?.length > 0) {
                let loginUser = tenantUser.find((user) => {
                    const isUserMatch = user.loginId === username || user.email === username;
                    if (!isUserMatch)
                        return false;
                    if (isOauthUser) {
                        return true;
                ...
                if (!loginUser) {
                    if (isOauthUser)
                        ...
                        throw new customException_1.NotFoundException('User not found');
                }
                appUser = JSON.parse(await this.redisService.getJsonData(appUserKey, process.env.CLIENTCODE)) || [];
                let loggedInUser = appUser.find((user) => loginUser?.userUniqueId === user?.userUniqueId);
                if (!loggedInUser) {
                    throw new customException_1.NotFoundException(`User did not have access to the application`);
                }
                ...
                if (isExpiredUser) {
                    throw new customException_1.NotAcceptableException('User access expired, Please contact administrator');
                }
                if (isOauthUser && !loggedInUser?.accessProfile?.length) {
                    throw new customException_1.UnauthorizedException(`User access pending for ${loggedInUser.loginId ?? loginUser?.email} , you'll be notified when approved`);
                }
                else if (!loggedInUser?.accessProfile?.length) {
                    throw new customException_1.UnauthorizedException(`User access pending for ${loggedInUser.loginId ?? loginUser?.email} , you'll be notified when approved`);
                }
                const userIndex = appUser.findIndex((user) => user.loginId === username &&
                ...
                await this.redisService.setJsonData(appUserKey, JSON.stringify(appUser), process.env.CLIENTCODE);
                ...
                if (fusionAuthLoginResponse && fusionAuthLoginResponse?.refresh_token) {
                    ...
                    refreshToken = await this.jwt.signAsync({ loginId: loggedInUser.loginId, client: tenant, type: 't' }, { secret: auth_secret, expiresIn: refreshTokenExpiryTime });
                }
                if (loggedInUser?.accessProfile &&
                ...
                if (loggedInUser?.accessProfile &&
                    ...
                    const filteredAccessprofile = accessProfileList.filter((t) => {
                        return loggedInUser.accessProfile.includes(t.accessProfile);
                    });
                    const filteredCombination = await this.transformToCombinations(filteredAccessprofile);
                ...
                await this.redisService.setJsonData(`CK:${tenant}:FNGK:AFP:FNK:${ufClientType}:CATK:${ag}:AFGK:${app}:AFK:session:AFVK:${version}:${loggedInUser?.loginId}_variables`, JSON.stringify(orpAccessObj), process.env.CLIENTCODE);
                return {
                    token,
                    authorized: true,
                ...
                await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH002', 'Invalid Credentials', 'LoginScreen', '', {
                    artifact: 'LoginScreen',
                    users: username,
                });
                throw new customException_1.NotFoundException(`Invalid Credentials`);
            }
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH002', error, 'LoginScreen', '', {
                artifact: 'LoginScreen',
                users: username,
            });
    ...
    async signInViaIAM(username, password, ufClientType, isOauthUser = false) {
        try {
            const url = `${fusionAuthBaseUrl}/oauth2/token`;
            const params = new URLSearchParams();
            ...
            if (!res.ok) {
                ...
                throw new customException_1.UnauthorizedException(JSON.parse(errorData)?.error_description ?? 'invalid credentials');
            }
            const fusionAuthLoginResponse = await res.json();
            const torusSignIn = await this.signIntoTorus(username, password, ufClientType, isOauthUser, fusionAuthLoginResponse);
            return torusSignIn;
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH001', error, 'LoginScreen', '', {
                artifact: 'LoginScreen',
                users: username,
            });
            await this.throwCustomException(error);
    ...
    async addSession(sessionObj, sessionListCacheKey) {
        try {
            const sessionListResponse = await this.redisService.getJsonData(sessionListCacheKey, process.env.CLIENTCODE);
            let sessionList = new Set();
            if (sessionListResponse) {
                const updatedSessionList = await this.checkSession(JSON.parse(sessionListResponse));
                sessionList = new Set(updatedSessionList);
            }
            await this.redisService.setJsonData(sessionListCacheKey, JSON.stringify([
                ...Array.from(sessionList).filter((s) => s?.sid !== sessionObj?.sid),
                sessionObj,
    ...
    comparePasswords(password, storedHash) {
        try {
            ...
            const testHash = (0, crypto_1.scryptSync)(password, salt, KEY_LENGTH);
            return (0, crypto_1.timingSafeEqual)(hashBuffer, testHash);
        }
        catch (error) {
            throw new common_1.BadGatewayException(error);
        }
    }
    async throwCustomException(error) {
        if (error instanceof customException_1.CustomException) {
            throw error;
        }
        throw new customException_1.CustomException('An unexpected error occurred', common_1.HttpStatus.INTERNAL_SERVER_ERROR);
    }
    ...
    async getTenantUser(tenantCode, client) {
        try {
            let key = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:TENANT:AFGK:${tenantCode}:AFK:PROFILE:AFVK:v1:users`;
            const res = JSON.parse(await this.redisService.getJsonData(key, client));
            return res || [];
        }
        catch (err) {
            throw new customException_1.UnauthorizedException('Invalid tenant key');
        }
    }
    async getAppUserList(tenant, ag, app, client) {
        try {
            if (!tenant || !ag || !app || !client) {
                return [];
            }
            const userCachekey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:users`;
            const responseFromRedis = JSON.parse(await this.redisService.getJsonData(userCachekey, client));
            return responseFromRedis || [];
        }
        catch (error) {
            throw new customException_1.UnauthorizedException('Please check credentials');
        }
    }
    async getTenantAppUser(tenant, client, ag, app) {
        try {
            let setAssignUsers = [];
            const tenantUser = await this.getTenantUser(tenant, client);
            const tenantAppUser = await this.getAppUserList(tenant, ag, app, client);
            tenantAppUser.filter((appUser) => tenantUser.some((tenantUser) => {
    ...
    async getAppSecurityData() {
        try {
            if (!tenant)
                throw new customException_1.BadRequestException('Either AppGroup or Application not available');
            ...
            for (let index = 0; index < cacheKeyArray.length; index++) {
                ...
                const data = await this.redisService.getJsonData(cacheKey, process.env.CLIENTCODE);
                if (data) {
            ...
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH008', error, 'UserScreen', '', {
                artifact: 'UserScreen',
                user: "anonymous user",
            });
            await this.throwCustomException(error);
    ...
    async getAPPSecurityTemplateData() {
        try {
            const responseFromRedis = await this.redisService.getJsonData(`CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:securityTemplate`, process.env.CLIENTCODE);
            const userResponse = await this.redisService.getJsonData(`CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:users`, process.env.CLIENTCODE);
            ...
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH009', error, 'UserScreen', '', {
                artifact: 'UserScreen',
    ...
    async getAppAccessProfiles() {
        try {
            ...
            if (responseFromRedis) {
                const accessProfileData = JSON.parse(responseFromRedis);
                accessProfileData.forEach((accessProfileObj) => {
                    var noOfProdService = 0;
                    accessProfileObj['orgGrp']?.forEach((orgGrp) => {
                        orgGrp['org']?.forEach((org) => {
                            org['psGrp']?.forEach((psGrp) => {
            ...
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH010', error, 'UserScreen', '', {
                artifact: 'UserScreen',
                users: 'anonymous user',
            });
            await this.throwCustomException(error);
    ...
    async postAppUserList(data) {
        try {
            if (!tenant || !data || !ag || !app || !process.env.CLIENTCODE) {
                throw new customException_1.BadRequestException('Invalid credentials');
            }
            const userCachekey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:users`;
            const responseFromRedis = JSON.parse(await this.redisService.getJsonData(userCachekey, process.env.CLIENTCODE));
            let dataExistOrNot = await (0, fusionAuth_api_1.FusionAuthUserApplicatonGet)(fusionAuthTenantId, data?.userUniqueId, fusionAuthApplicationId);
            if (responseFromRedis == null || responseFromRedis?.length == 0) {
                if (defaultAuth === 'fusionauth' &&
                    ...
                    if (dataExistOrNot?.isNotExist) {
                        await (0, fusionAuth_api_1.FusionAuthApplicatonAssign)(fusionAuthTenantId, data?.userUniqueId, fusionAuthApplicationId, data.firstName, data.lastName, data.loginId, data.email, 'POST', data?.accessProfile || []);
                    }
                }
                return await this.redisService.setJsonData(userCachekey, JSON.stringify([data]), process.env.CLIENTCODE);
            }
            else {
                let userExist = false;
                let updateIndex = null;
                responseFromRedis.forEach((user, index) => {
                ...
                if (userExist) {
                    ...
                    if (defaultAuth === 'fusionauth' &&
                        ...
                        if (dataExistOrNot?.isNotExist) {
                            ...
                            await (0, fusionAuth_api_1.FusionAuthApplicatonAssign)(fusionAuthTenantId, data?.userUniqueId, fusionAuthApplicationId, data.firstName, data.lastName, data.loginId, data.email, 'PUT', data?.accessProfile || []);
                        }
                    }
                    let uniqueData = responseFromRedis?.filter((obj) => 'userUniqueId' in obj);
                    uniqueData = uniqueData.filter((item, index, self) => index ===
                        self.findIndex((t) => t.userUniqueId === item.userUniqueId));
                    const res = await this.redisService.setJsonData(userCachekey, JSON.stringify(uniqueData), process.env.CLIENTCODE);
                    return res;
                }
                else {
                    if (defaultAuth === 'fusionauth' &&
                    ...
                    let uniqueData = responseFromRedis;
                    uniqueData = uniqueData.filter((item, index, self) => index ===
                        self.findIndex((t) => t.userUniqueId === item.userUniqueId));
                    const res = await this.redisService.setJsonData(userCachekey, JSON.stringify(uniqueData), process.env.CLIENTCODE);
                    return res;
            ...
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH011', error, 'UserScreen', '', {
                artifact: 'UserScreen',
                users: 'anonymous user',
            });
            await this.throwCustomException(error);
    ...
    async setJson(key, data) {
        try {
            return await this.redisService.setJsonData(key, JSON.stringify(data), process.env.CLIENTCODE);
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH012', error, key, '', {
                artifact: 'UserScreen',
                users: 'anonymous user',
            });
            await this.throwCustomException(error);
    ...
    async appUserAddition(data, isFusionAuth = false) {
        try {
            if (!tenant || !ag || !app || !data) {
                throw new customException_1.BadRequestException('Invalid input parameters');
            }
            const userCachekey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:users`;
            ...
            const clientProfile = JSON.parse(await this.redisService.getJsonData(clientProfileResourceKey, process.env.CLIENTCODE));
            const { email, firstName, lastName, password, loginId } = data;
            const resForClientUserAddition = await this.redisService.getJsonData(`CK:TRL:FNGK:AFR:FNK:PORTAL:CATK:EMAILTEMPLATE:AFGK:TORUS:AFK:CLIENTUSERADDITION:AFVK:v1:TPI`, process.env.CLIENTCODE);
            const clientUserAddition = JSON.parse(resForClientUserAddition);
            const updatedSubject = clientUserAddition.subject.replaceAll('${clientProfile.clientName}', `${clientProfile.Name}`);
            const updateclientUserAdditionHtml = clientUserAddition.html
            ...
            transporter.sendMail(mailOptions, async (error, info) => {
                if (error) {
                    throw new customException_1.ForbiddenException('There is an issue with sending otp');
                }
                else {
            ...
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH013', error, 'UserScreen', '', {
                artifact: 'UserScreen',
                users: 'anonymous user',
            });
    ...
    async getDFS(fileUrl, enableEncryption) {
        try {
    ...
    async uploadImage(file, bucketFoldername, folderPath, filename, enableEncryption) {
        try {
            ...
            form.append('file', stream_1.Readable.from(encryptedBuffer), {
                filename: fileName,
                contentType: file.mimetype || 'application/octet-stream',
            });
            const uploadUrl = `${process.env.SEAWEED_OUTPUT_HOST?.replace(/\/$/, '')}/buckets/${bucket}/${subFolder}/${fileName}`;
            ...
            if (res.status === 201) {
                ...
                throw new customException_1.ConflictException(res.data || 'Error occurred while uploading file');
            }
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH014', error, 'UserScreen', '', {
                artifact: 'UserScreen',
                users: 'anonymous user',
            });
            await this.throwCustomException(error);
    ...
    async readAMDKey(key, token) {
        const valueObj = await this.commonService.readAPI(key, 'redis', 'redis');
        if (valueObj) {
            return valueObj;
        }
        else {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'TG027', 'setupKey not found', key, token);
            throw new customException_1.NotFoundException('data not found');
        }
    }
    async getResetPasswordOtp(email) {
        try {
            if (!email)
                throw new customException_1.BadRequestException('email is required');
            const userCachekey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:users`;
            const otpCacheKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:otp`;
            const userResponse = await this.redisService.getJsonData(userCachekey, process.env.CLIENTCODE);
            if (!userResponse)
                throw new customException_1.NotFoundException('no data found');
            const userList = userResponse ? JSON.parse(userResponse) : [];
            const foundedUser = userList.find((user) => user.email.toLowerCase() === email.toLowerCase());
            if (!foundedUser)
                throw new customException_1.NotFoundException('user not found');
            const otpTemplateFromRedis = await this.redisService.getJsonData('CK:TRL:FNGK:AFR:FNK:PORTAL:CATK:EMAILTEMPLATE:AFGK:TORUS:AFK:RESETPASSWORDOTP:AFVK:v1:TPI', 'TORUS');
            ...
            const updatedTemplateHtml = resetOtpTemplate.html
                .replace('${name}', `${capitalizeFirstLetter(foundedUser.firstName ?? email)} ${capitalizeFirstLetter(foundedUser.lastName ?? '')}`)
                .replace('${otp}', `${otp}`)
            ...
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH003', error, 'ForgotPasswordScreen', '', {
    ...
    async verifyOtp(email, otp) {
        try {
            if (!email || !otp)
                throw new customException_1.BadRequestException('email or otp is required');
            const otpCacheKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:otp`;
            const otpJsonFromRedis = await this.redisService.getJsonData(otpCacheKey, process.env.CLIENTCODE);
            if (!otpJsonFromRedis)
                throw new customException_1.NotFoundException('otp not found');
            const otpJson = JSON.parse(otpJsonFromRedis);
            const existingIndex = otpJson.findIndex((ele) => ele.email == email && ele.otp == otp);
            if (existingIndex == -1)
                throw new customException_1.NotFoundException('invalid otp');
            otpJson.splice(existingIndex, 1);
            await this.redisService.setJsonData(otpCacheKey, JSON.stringify(otpJson), process.env.CLIENTCODE);
            return true;
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH004', error, 'ForgotPasswordScreen', '', {
                artifact: 'ForgotPasswordScreen',
                users: email.split("@")[0],
            });
            await this.throwCustomException(error);
    ...
    async resetPassword(email, password) {
        try {
            if (!email || !password) {
                throw new customException_1.BadRequestException('Please provide valid email and password');
            }
            const tenantUserKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:TENANT:AFGK:${tenant}:AFK:PROFILE:AFVK:v1:users`;
            const tenantUserResponse = await this.redisService.getJsonData(tenantUserKey, process.env.CLIENTCODE);
            if (!tenantUserResponse) {
                throw new customException_1.NotFoundException('No data found');
            }
            const tenantList = JSON.parse(tenantUserResponse);
            const index = tenantList.findIndex((user) => user.email.toLowerCase() === email.toLowerCase());
            if (index === -1) {
                throw new customException_1.NotFoundException('User not found');
            }
            ...
            if (process.env.DEFAULT_AUTHENTICATION === 'fusionauth') {
                ...
                if (!fusionAuthTenantId || !uniqueId) {
                    throw new customException_1.NotFoundException(`Missing FusionAuth tenantId or userUniqueId`);
                }
                const value = await this.handleFusionResetPassWord(fusionAuthTenantId, password, uniqueId);
                console.log("value", value);
                if (value.status !== 200) {
                    throw new customException_1.UnauthorizedException(value?.error ?? 'FusionAuth password update failed');
                }
            }
            tenantUser.password = this.hashPassword(password);
            tenantList.splice(index, 1, tenantUser);
            await this.redisService.setJsonData(tenantUserKey, JSON.stringify(tenantList), process.env.CLIENTCODE);
            return 'Password updated successfully';
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH005', error, 'ForgotPasswordScreen', '', {
                artifact: 'ForgotPasswordScreen',
                users: email.split("@")[0],
            });
            await this.throwCustomException(error);
    ...
    async handleFusionResetPassWord(fusionAuthTenantId, password, uniqueId) {
        try {
            ...
            if (!res.ok) {
                ...
                throw new customException_1.UnauthorizedException(`FusionAuth password update failed: ${error}`);
            }
            const data = await res.json();
            return {
                status: res.status,
    ...
    async sendMailOTP(email) {
        try {
            if (!email) {
                throw new customException_1.BadRequestException('Not enough data to continue');
            }
            const otp = Math.floor(100000 + Math.random() * 900000);
            const responseFromRedis = await this.redisService.getJsonData('CK:TRL:FNGK:AFR:FNK:PORTAL:CATK:EMAILTEMPLATE:AFGK:TORUS:AFK:MAILVERIFICATIONOTP:AFVK:v1:TPI', process.env.CLIENTCODE);
            const verificationTemplate = JSON.parse(responseFromRedis);
            const updatedTemplate = verificationTemplate.text
            ...
            const mailOptions = {
                ...
                html: verificationTemplate.html
                    .replace('${email}', fabricatedUserName.charAt(0).toUpperCase() +
    ...
    async getEndPoints(input) {
        try {
            ...
            const contentType = typeMap[input.type] ||
                (() => {
            ...
            const getContentTypes = (endpointData, statusCode) => {
                ...
                if (ref || endpointData?.requestBody?.[statusCode]?.$ref) {
                    ...
                    const componentContent = specData?.components?.[pathParameter]?.[responseParameter]?.content;
                    if (componentContent)
                        return Object.keys(componentContent);
                }
                return [];
            };
            ...
            if (data?.paths &&
                ...
                if (endPointCategory && endPointCategory.length > 0) {
                    ...
                    for (let i = 0; i < endPointCategory.length; i++) {
                        let uniqueNodeid = (0, uuid_1.v4)().replace(/-/g, '');
                        ...
                        if (data?.paths[endPoint][methodName]) {
                            ...
                            if (endpointData?.responses) {
                                ...
                                for (let k = 0; k < Object.keys(responseArr).length; k++) {
                                    if (data?.paths[endPoint][methodName]?.responses[Object.keys(responseArr)[k]]?.content) {
                                        let oldContent;
                                        if (contentJson?.responses[Object.keys(responseArr)[k]]
                                            ?.content[contentType]) {
                                            oldContent =
                                                contentJson?.responses[Object.keys(responseArr)[k]]
                                                    ?.content[contentType];
                                        }
                                        else if (contentJson?.responses[Object.keys(responseArr)[k]]
                                            ?.content['*/*']) {
                                            oldContent =
                                                contentJson?.responses[Object.keys(responseArr)[k]]
                                                    ?.content['*/*'];
                        ...
                        if (requestRef) {
                            ...
                            var ModelArr = [];
                            await this.getModel(schema, requestParameter, ModelArr, contentType);
                            var RequestDto = {};
                            await this.getReferenceModel(schema, requestParameter, RequestDto, contentType);
                        }
                        if (methodName == 'get') {
                        ...
                        if (responseRef) {
                            ...
                            let pathParameter = responseRef?.split('/')[(responseRef?.split('/')).length - 2];
                            var ResponseModelArr = [];
                            await this.getResponseModel(schema, pathParameter, responseParameter, ResponseModelArr, contentType);
                            var ResponseDto = {};
                            await this.getReferenceResponseModel(schema, pathParameter, responseParameter, ResponseDto, contentType);
                        }
                        let key;
                        let newVersion;
                        ...
                        if (ResponseModelArr && ResponseModelArr.length > 0) {
                            ResponseModelArr = ResponseModelArr.filter((value, index, self) => index ===
                                self.findIndex((t) => JSON.stringify(t) === JSON.stringify(value)));
                        }
                        if (ModelArr && ModelArr.length > 0) {
                            ModelArr = ModelArr.filter((value, index, self) => index ===
                                self.findIndex((t) => JSON.stringify(t) === JSON.stringify(value)));
                        }
                        var requestSchema = data.paths[endPoint][methodName]?.requestBody?.content[contentType]?.schema;
                            ...
                            const transformProperties = (properties) => {
                                ...
                                if (Array.isArray(properties) && properties.length > 0) {
                                    for (let i = 0; i < properties.length; i++) {
                                        ...
                                        else if (properties[i]?.schema?.type == 'number' ||
                                        ...
                                        if (prop.type === 'object') {
                                            ...
                                            for (let item of prop.oneOf) {
                                                if (item.type === 'object') {
                                                    ...
                                                    Object.assign(result[key], transformProperties(item?.properties));
                                                }
                                            }
                                        }
                                        else if (prop.allOf) {
                        ...
                        var requestJsonFromSchema = transformSchema(requestSchema);
                        var parameterJsonFromSchema = transformSchema(parameterSchema);
                        Object.assign(wholeResponseModel, ResponseDto);
                        let ndp = await this.redisService.setJsonData(key + ':NDP', JSON.stringify({
                            [uniqueNodeid]: {
                                nodeId: uniqueNodeid,
                        ...
                        let nds = await this.redisService.setJsonData(key + ':NDS', JSON.stringify([ndsData]), collectionName);
                        let nde = await this.redisService.setJsonData(key + ':NDE', JSON.stringify([]), collectionName);
                        let afi = await this.redisService.setJsonData(key + ':AFI', JSON.stringify({
                            executionMode: '',
                            createdBy: loginId,
                            createdOn: new Date(),
                            updatedBy: loginId,
    ...
    async replaceRefs(schema) {
        const components = schema.components || {};
        const resolveRefs = (obj, propName = '') => {
            if (Array.isArray(obj)) {
    ...
    async getModel(data, requestParameter, nestedModelArr, contentType) {
        try {
            const processProperties = async (properties, dto) => {
                if (properties) {
                    let propArr = Object.keys(properties);
                    let propval = Object.values(properties);
                    if (propArr?.length > 0) {
                        for (let i = 0; i < propArr.length; i++) {
                            if (propval[i]?.properties &&
                                    ...
                                    if (propval[i].oneOf[j].$ref) {
                                        ...
                                        if (data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type)
                                            dto[requestParameter][propArr[i]] =
                                                data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type;
                                        else
                                            dto[requestParameter][propArr[i]] = 'object';
                                        await this.getModel(data, nestedSchema[nestedSchema.length - 1], nestedModelArr, contentType);
                                    }
                                }
                            }
            ...
            if (data?.components?.schemas[requestParameter]) {
                if (data?.components?.schemas[requestParameter]?.properties ||
                    ...
                    if (data?.components?.schemas[requestParameter]?.properties) {
                        var propertyArr = Object.keys(data?.components?.schemas[requestParameter]?.properties);
                        var type = Object.values(data?.components?.schemas[requestParameter]?.properties);
                    }
                    else if (data?.components?.schemas[requestParameter]?.items?.properties) {
                        var propertyArr = Object.keys(data?.components?.schemas[requestParameter]?.items?.properties);
                        var type = Object.values(data?.components?.schemas[requestParameter]?.items?.properties);
                    }
                    ...
                    if (propertyArr?.length > 0) {
                        for (let i = 0; i < propertyArr.length; i++) {
                            if (type[i]?.properties) {
                                ...
                                let nestedSchema = type[i]?.content?.[contentType]?.schema?.$ref.split('/');
                                await this.getModel(data, nestedSchema[nestedSchema.length - 1], nestedModelArr, contentType);
                            }
                    ...
                    let nestedSchema = data?.components?.schemas[requestParameter]?.items.$ref.split('/');
                    await this.getModel(data, nestedSchema[nestedSchema.length - 1], nestedModelArr, contentType);
                }
                else if (data?.components?.schemas?.[requestParameter]?.items?.properties) {
                    await processProperties(data?.components?.schemas?.[requestParameter]?.items?.properties, dto);
                }
                else if (!data?.components?.schemas[requestParameter]?.properties &&
    ...
    async getReferenceModel(data, requestParameter, ReferenceResponseDto, contentType) {
        try {
            const processProperties = async (properties, ReferenceResponseDto) => {
                if (properties) {
                    let propArr = Object.keys(properties);
                    let propval = Object.values(properties);
                    if (propArr?.length > 0) {
                        for (let i = 0; i < propArr.length; i++) {
                            if (propval[i]?.properties &&
                                Object.keys(propval[i]?.properties).length > 0) {
                                await processProperties(propval[i]?.properties, ReferenceResponseDto);
                            }
                            else if (propval[i]?.oneOf) {
                                for (let j = 0; j < propval[i].oneOf.length; j++) {
                                    if (propval[i].oneOf[j].$ref) {
                                        let nestedSchema = propval[i].oneOf[j].$ref.split('/');
                                        await this.getReferenceModel(data, nestedSchema[nestedSchema.length - 1], ReferenceResponseDto, contentType);
                                    }
                                }
                            }
                            else if (propval[i]?.allOf) {
                                ...
                                await processProperties(propval[i]?.items?.properties, ReferenceResponseDto);
                            }
                            else if (propval[i]?.items?.$ref) {
            ...
            if (data?.components?.schemas[requestParameter]) {
                if (data?.components?.schemas[requestParameter]?.properties) {
                    ...
                    if (propertyArr?.length > 0) {
                        for (let i = 0; i < propertyArr.length; i++) {
                            if (type[i]?.properties) {
                                await processProperties(type[i]?.properties, ReferenceResponseDto);
                            }
                            else if (type[i]?.items && type[i].items?.$ref) {
    ...
    async getResponseModel(data, pathParameter, responseParameter, nestedModelArr, contentType) {
        try {
            const processProperties = async (properties, dto) => {
                if (properties) {
                    let propArr = Object.keys(properties);
                                ...
                                for (let j = 0; j < propval[i].oneOf.length; j++) {
                                    if (propval[i].oneOf[j].$ref) {
                                        ...
                                        if (data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type)
                                            ...
                                            dto[responseParameter][propArr[i]] = 'object';
                                        await this.getResponseModel(data, nestedSchema[nestedSchema.length - 2], nestedSchema[nestedSchema.length - 1], nestedModelArr, contentType);
                                    }
                                }
                            }
                            else if (propval[i]?.allOf) {
            ...
            if (data?.components?.[pathParameter]?.[responseParameter]) {
                if (data?.components?.[pathParameter]?.[responseParameter]?.content?.[contentType]?.schema?.$ref) {
                    let nestedSchema = data?.components?.[pathParameter]?.[responseParameter]?.content[contentType].schema.$ref.split('/');
                    await this.getResponseModel(data, nestedSchema[nestedSchema.length - 2], nestedSchema[nestedSchema.length - 1], nestedModelArr, contentType);
                }
                else if (data?.components?.[pathParameter]?.[responseParameter]?.properties ||
                    data?.components?.[pathParameter]?.[responseParameter]?.items
                        ?.properties) {
                    if (data?.components?.[pathParameter]?.[responseParameter]?.properties) {
                        var propertyArr = Object.keys(data?.components?.[pathParameter]?.[responseParameter]
                            ?.properties);
                        var type = Object.values(data?.components?.[pathParameter]?.[responseParameter]
                            ?.properties);
                    }
                    else if (data?.components?.[pathParameter]?.[responseParameter]?.items
                        ?.properties) {
                        var propertyArr = Object.keys(data?.components?.[pathParameter]?.[responseParameter]?.items
                            ?.properties);
                        var type = Object.values(data?.components?.[pathParameter]?.[responseParameter]?.items
                            ?.properties);
                    }
                                ...
                                if (!type[i]?.properties && !type[i]?.items) {
                                    if (data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type)
                                        dto[responseParameter][propertyArr[i]] =
                                            data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.type;
                                    else if (data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.oneOf?.[0]?.type)
                                        dto[responseParameter][propertyArr[i]] =
                                            data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.oneOf[0]?.type;
                                    else if (data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.allOf?.[0]?.type)
                                        dto[responseParameter][propertyArr[i]] =
                                            data?.components?.[nestedSchema[nestedSchema.length - 2]]?.[nestedSchema[nestedSchema.length - 1]]?.allOf[0]?.type;
                                }
                                await this.getResponseModel(data, nestedSchema[nestedSchema.length - 2], nestedSchema[nestedSchema.length - 1], nestedModelArr, contentType);
                            }
                            else if (type[i].items?.properties) {
                ...
                else if (data?.components?.[pathParameter]?.[responseParameter]?.items?.$ref) {
                    ...
                    let nestedSchema = data?.components?.[pathParameter]?.[responseParameter]?.items.$ref.split('/');
                    await this.getResponseModel(data, nestedSchema[nestedSchema.length - 2], nestedSchema[nestedSchema.length - 1], nestedModelArr, contentType);
                }
                else if (data?.components?.[pathParameter]?.[responseParameter]?.items
                    ?.properties) {
                    await processProperties(data?.components?.[pathParameter]?.[responseParameter]?.items
                        ?.properties, dto);
                }
                else if (!data?.components?.[pathParameter]?.[responseParameter]?.properties &&
                    !data?.components?.[pathParameter]?.[responseParameter]?.items &&
                ...
                else if (data?.components?.[pathParameter]?.[responseParameter]?.allOf) {
                    let allOf = data?.components?.[pathParameter]?.[responseParameter]?.allOf;
                    for (let j = 0; j < allOf.length; j++) {
                        var dto = { [responseParameter]: {} };
                        if (allOf[j].$ref) {
                            let nestedSchema = allOf[j]?.$ref.split('/');
                ...
                else if (data?.components?.[pathParameter]?.[responseParameter]?.oneOf) {
                    let oneOf = data?.components?.[pathParameter]?.[responseParameter]?.oneOf;
    ...
    async getReferenceResponseModel(data, pathParameter, responseParameter, ResponseDto, contentType) {
        try {
            const processProperties = async (properties, ResponseDto) => {
                if (properties) {
                    let propArr = Object.keys(properties);
                    let propval = Object.values(properties);
                    if (propArr?.length > 0) {
                            ...
                            if (propval[i]?.properties &&
                                ...
                                for (let j = 0; j < propval[i].oneOf.length; j++) {
                                    if (propval[i].oneOf[j].$ref) {
                                        ...
                                        await this.getReferenceResponseModel(data, nestedSchema[nestedSchema.length - 2], nestedSchema[nestedSchema.length - 1], ResponseDto, contentType);
                                    }
                                }
                            }
                            else if (propval[i]?.allOf) {
                                ...
                                await processProperties(propval[i]?.items?.properties, ResponseDto);
                            }
                            else if (propval[i]?.items?.$ref) {
    ...
    async notifyUserAccessPending(oauthUser, userList) {
        try {
            const adminList = userList
                ?.filter((user) => user?.isAppAdmin === true)
                .map((user) => user?.email);
            let mailOptions = {};
            const emailTemplateResponseFromRedis = await this.redisService.getJsonData(`CK:TRL:FNGK:AFR:FNK:PORTAL:CATK:EMAILTEMPLATE:AFGK:TORUS:AFK:OAUTHUSERACCESSREQUEST:AFVK:v1:TPI`, process.env.CLIENTCODE);
            ...
            if (adminList.length) {
                mailOptions = {
                    ...
                    html: template.html
                        ...
                        .replaceAll('${name}', oauthUser?.name)
                        .replaceAll('${email}', oauthUser?.email)
                        .replaceAll('${appUrl}', process.env.BE_URL.substring(0, process.env.BE_URL.lastIndexOf("/"))),
                };
            }
            else {
                mailOptions = {
    ...
    async oauthSignIn(user) {
        try {
            if (!user) {
                throw new customException_1.BadRequestException('Account details not enough to continue');
            }
            if (user?.provider == 'fusionauth') {
                const fusionauthUser = await (0, fusionAuth_api_1.FusionAuthUserGet)(user?.providerAccountId);
            ...
            if (isExistingUser) {
                ...
                const userListResponse = await this.redisService.getJsonData(userCachekey, process.env.CLIENTCODE);
                const userList = userListResponse ? JSON.parse(userListResponse) : [];
                await this.redisService.setJsonData(userCachekey, JSON.stringify([...userList, userObject]), process.env.CLIENTCODE);
                await this.notifyUserAccessPending(user, userList);
                return await this.signIntoTorus(user?.email, '', user?.ufClientType, true);
            }
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH015', error, 'LoginScreen', '', {
                artifact: 'LoginScreen',
                users: user,
            });
            await this.throwCustomException(error);
    ...
    async AppSecurityTemplateData(data) {
        try {
            if (tenant && data) {
                const clientCode = process.env.CLIENTCODE;
                let defaultAuthenticationMethod = 'nextAuth';
                let applicationUniqueId = '';
                let setupData = JSON.parse(await this.redisService.getJsonData(`CK:TGA:FNGK:SETUP:FNK:SF:CATK:TENANT:AFGK:${tenant}:AFK:PROFILE:AFVK:v1:tpc`, clientCode));
                let newData = [];
                setupData?.AG.map((AppGrp) => {
                    if (AppGrp?.code == ag) {
                        AppGrp?.APPS.map((App) => {
                            if (App?.code == app) {
                ...
                let existRoles = JSON.parse(await this.redisService.getJsonData(`CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:securityTemplate`, clientCode)) || [];
                existRoles.map((existRoleObj) => {
                    data.map((comingRoleObj) => {
                        if (existRoleObj?.roleUniqueId == comingRoleObj?.roleUniqueId &&
                ...
                const deletedRoles = existRoles.filter((r) => !inputTenantIds.includes(r.roleUniqueId));
                if (defaultAuthenticationMethod == 'fusionauth' &&
                    applicationUniqueId) {
                    for (const roleObj of deletedRoles) {
                        await (0, fusionAuth_api_1.FusionAutRoleCRUDAlongWithApp)(applicationUniqueId, roleObj?.roleUniqueId, roleObj?.accessProfile, 'DELETE');
                    }
                    for (const roleObj of newData) {
                        if (roleObj?.isForEdit) {
                            await (0, fusionAuth_api_1.FusionAutRoleCRUDAlongWithApp)(applicationUniqueId, roleObj?.roleUniqueId, roleObj?.accessProfile, 'DELETE');
                            await (0, fusionAuth_api_1.FusionAutRoleCRUDAlongWithApp)(applicationUniqueId, roleObj?.roleUniqueId, roleObj?.accessProfile, 'POST');
                        }
                        else {
                            await (0, fusionAuth_api_1.FusionAutRoleCRUDAlongWithApp)(applicationUniqueId, roleObj?.roleUniqueId, roleObj?.accessProfile, 'POST');
                        }
                ...
                return await this.redisService.setJsonData(`CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:securityTemplate`, JSON.stringify(data), clientCode);
            }
            else {
                throw new customException_1.BadRequestException('Please provide all necessary credentials');
            }
        }
        catch (error) {
            await this.commonService.errorLog('Technical', 'AK', 'Fatal', 'AUTH016', error, 'UserScreen', '', {
                artifact: 'UserScreen',
                users: 'anonymous user',
            });
            await this.throwCustomException(error);
    ...
    async getAccessProfileForArtifact(key, clientCode, token) {
        try {
            const UO = await this.commonService.readAPI(key, clientCode, token);
            let templateArray = UO?.securityData?.accessProfile || [];
            let allowedAccessProfile = [];
            templateArray.map((profile) => {
    ...
    async navbarDataPreparation(data, clientCode, token) {
        const result = [];
        for (const group of data) {
            ...
            for (const screen of group.screens) {
                ...
                if (screen.UF != 'Logs Screen' && screen.UF != 'User Screen') {
                    allowedAccessProfile = await this.getAccessProfileForArtifact(screen.UF + ':UO', clientCode, token);
                }
    ...
    async getNavbarData(key, clientCode, token) {
        let webAssemblerData = await this.commonService.readAPI(key, process.env.CLIENTCODE, token);
        let screenDetailsForNav = await this.screenDetailsData(webAssemblerData.webArtifacts);
        let navbarData = await this.navbarDataPreparation(screenDetailsForNav, clientCode, token);
        return navbarData;
    }
    async postTenantUser(userDetail) {
        let tenantUserKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:TENANT:AFGK:${tenant}:AFK:PROFILE:AFVK:v1:users`;
        const appUserKey = `CK:TGA:FNGK:SETUP:FNK:SF:CATK:${tenant}:AFGK:${ag}:AFK:${app}:AFVK:v1:users`;
        let tenantUser = JSON.parse(await this.redisService.getJsonData(tenantUserKey, process.env.CLIENTCODE));
        if (tenantUser?.length) {
            const isExists = tenantUser.find((allUser) => allUser?.email == userDetail?.email);
            if (!isExists) {
                let temp = userDetail?.email?.split('@').at(0) || '';
                let postOneUser = {
                ...
                await this.redisService.setJsonData(tenantUserKey, JSON.stringify(tenantUser), process.env.CLIENTCODE);
                postOneUser = {
                    ...postOneUser,
                    noOfProductsService: 0,
                    accessProfile: [],
            ...
            await this.redisService.setJsonData(tenantUserKey, JSON.stringify([postOneUser]), process.env.CLIENTCODE);
            postOneUser = {
                ...postOneUser,
                noOfProductsService: 0,
                accessProfile: [],
...
exports.UfService = UfService = __decorate([
    ...
    __metadata("design:paramtypes", [jwt_services_1.JwtServices,
        jwt_1.JwtService,
        ruleService_1.RuleService,
