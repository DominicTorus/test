...
var DynamicFlowService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.DynamicFlowService = void 0;
...
let DynamicFlowService = DynamicFlowService_1 = class DynamicFlowService {
    constructor(redisService, listenerService, jwtService, CommonService, lockservice, processor) {
        ...
        this.statickeyword = ['get', 'post', 'patch', '200', '201', '202', '204', '400', '401', '403', '404', '500', 'requestBody', '*/*', 'responses', 'content', 'application/json', 'application/xml', 'text/plain', 'application/jwt', 'application/json; charset=utf-8', 'schema', 'properties', 'allOf', 'oneOf', 'inputschema', 'outputschema',];
        this.numberArr = Array.from({ length: 101 }, (_, i) => i.toString());
        this.logger = new common_1.Logger(DynamicFlowService_1.name);
    }
    async DynamicFlowProcess(input) {
        ...
        for (var j = 0; j < poNode.length; j++) {
            ...
            if (nodeType == 'decisionnode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    if (!poNode[j].rule || Object.values(poNode[j].rule).length == 0) {
                        throw new customException_1.CustomException('Rule is required for decision node', 404);
                    }
                    let decisionRes;
                    RCMresult = await this.CommonService.getRuleCodeMapper(poNode[j], inputparam, processedKey + upId, currentFabric, SessionInfo);
                    if (RCMresult) {
            ...
            if ((nodeType == 'apinode' || nodeType == 'googlefileapinode') && poNode[j].nodeId == nodeId) {
                ...
                try {
                    if (currentFabric == 'PF-SCDL' && poNode[j].nodeId == poNode[1].nodeId) {
                        ...
                        let apires, headerRole;
                        if (customConfig) {
                            if (rule?.approvalProcess) {
                                let rulekey = rule?.ruleKey;
                                if (!rulekey)
                                    throw new customException_1.CustomException(pfjson[i].nodeName + ' rulekey key not found', 404);
                                let ruleConfig = JSON.parse(await this.redisService.getJsonData(rulekey, collectionName));
                                if (!ruleConfig || Object.keys(ruleConfig).length == 0)
                                    throw new customException_1.CustomException(pfjson[i].nodeName + ' Reference key value not found', 404);
                                let rulejson = Object.values(ruleConfig)[0];
                                let rulecheck = await this.CommonService.getRuleCodeMapper(rulejson, inputparam, processedKey + upId, currentFabric, SessionInfo);
                                ...
                                if (methodName) {
                                    ...
                                    if (methodName == 'get') {
                                        if (serverUrl && endPoint) {
                                            ...
                                            throw new customException_1.CustomException('Endpoint not found', 404);
                                        }
                                    }
                                ...
                                if (childInsertArr?.length > 0) {
                                            ...
                                            if (methodName == 'get') {
                                                ...
                                                if (apiUrl) {
                                                    ...
                                                    if (contentType == 'application/json' && mapObj && Object.keys(mapObj).length > 0) {
                                                        ...
                                                        const xml = (0, xml_js_1.json2xml)(jsonString, { compact: true, spaces: 4 });
                                                    ...
                                                    try {
                                                        if (lockDetails && (lockDetails.lockMode).toLowerCase() == 'single' && lockDetails.ttl) {
                                                            ...
                                                            if (await this.redisService.exist(processedKey + 'lock', collectionName)) {
                                                                ...
                                                                if (isProcessing.tablename == tableName && isProcessing.primarykey == primaryKey && isProcessing.lockflag == true) {
                                                                    ...
                                                                    await this.redisService.setJsonData(processedKey + 'lock', JSON.stringify(true), collectionName, 'lockflag');
                                                            }
                                                            else {
                                                        ...
                                                        if (lockDetails) {
                                                            if (lockDetails.ttl && JSON.stringify(error).includes('quorum')) {
                                                                throw new customException_1.CustomException('Resource locked by other user', 423);
                                                            }
                                                            if (lock) {
                                                                await this.lockservice.releaseLock(lock);
                                    ...
                                    if (apiUrl) {
                                        ...
                                        if (contentType == 'text/plain') {
                                            ...
                                            throw new customException_1.CustomException(`Mapping was required in ${poNode[j].nodeName}`, 400);
                                        }
                                        if (apiResult) {
                                            if (apiResult.statusCode == 201 || apiResult.statusCode == 200) {
                                                apiResult = apiResult?.result;
                ...
                try {
                    ...
                    if (manualQry) {
                        ...
                        if (internalEdges && internalEdges.hasOwnProperty(poNode[j].nodeId)) {
                            ...
                            throw new customException_1.CustomException('Mapping was required', 404);
                        }
                        if (!filterData || filterData.length == 0) {
                            filterData = [];
                    ...
                    if (currentFabric == 'PF-SCDL' && poNode[j].nodeId == poNode[1].nodeId) {
                        ...
                        if (!childInsertArr && !textobj) {
                            if (inputparam && (Object.keys(inputparam).length > 0 || inputparam.length > 0)) {
                                ...
                                throw new customException_1.CustomException('Stream Config Mapping was Required', 404);
                            }
                        }
                        if (!isStatic) {
                            let streaminfo = childInsertArr[0].streaminfo;
                            if (!streaminfo || Object.keys(streaminfo).length == 0)
                                throw new customException_1.CustomException('Stream Config Mapping was Required', 404);
                        ...
                        if (storageType?.toLowerCase() == 'externel') {
                            if (oprname == 'read') {
                                ...
                                if (startOfToday && endOfToday) {
                                    ...
                                    if (await redis.call('EXISTS', streamName)) {
                                        var grpInfo = await redis.xinfo('GROUPS', streamName);
                                        if (grpInfo.length == 0) {
                                        ...
                                        var result = await redis.xreadgroup('GROUP', consumerGroupName, consumerName, 'STREAMS', streamName, '>');
                                        let streamData = [];
                                        if (result) {
                                    ...
                                    for (let a = 0; a < childInsertArr.length; a++) {
                                        if (childInsertArr[a].hasOwnProperty('entryId')) {
                                            ...
                                            let entryData = await redis.xrange(streamName, entryId, entryId);
                                ...
                                if (streamArr?.length == 0)
                                    throw new customException_1.CustomException('No Data available to read from Processor', 404);
                            }
                            ...
                            if (oprname == 'read') {
                                ...
                                if (streamArr?.length == 0)
                                    throw new customException_1.CustomException(`No Data available to read in ${streamName}, ${poNode[j].nodeName}`, 404);
                            }
                            else if (oprname == 'write') {
                    ...
                    if (currentFabric == 'PF-SCDL' && poNode[j].nodeId == poNode[1].nodeId) {
                        ...
                        if (!childInsertArr && !textobj) {
                            if (inputparam && (Object.keys(inputparam).length > 0 || inputparam.length > 0)) {
                                ...
                                throw new customException_1.CustomException('Kafka message data mapping is required for produce operation', 404);
                            }
                        }
            ...
            if (nodeType == 'filenode' && poNode[j].nodeId == nodeId) {
                ...
                try {
                    ...
                    if (!isStatic && (!childInsertArr || !textobj))
                        throw new customException_1.CustomException('File Config Mapping was Required', 404);
                    if (oprname && oprkey.includes(oprname)) {
                        let fileinfo = childInsertArr[0].fileinfo;
                        if (!isStatic && (!fileinfo || Object.keys(fileinfo).length == 0))
                            throw new customException_1.CustomException('File Config Mapping was Required', 404);
                        if (fileinfo) {
                        ...
                        if (oprname === 'read') {
                            ...
                            if (!fileres || fileres?.status != 201) {
                                throw new customException_1.CustomException('write operation failed', 500);
                            }
                        }
                        let s_obj = {};
                        let sarr = [];
            ...
            if (nodeType == 'subflow_node' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    if (customConfig) {
                        ...
                        if (!PfdKey)
                            throw new customException_1.CustomException('PFD key not found', 404);
                        ifoObj = await this.ifoAssign(poJson?.internalMappingNodes, poNode[j].nodeId);
                        if (ifoObj && Object.keys(ifoObj).length > 0)
                            await this.redisService.setJsonData(processedKey + upId + ':NPV:' + poNode[j].nodeName + '.PRO', JSON.stringify(ifoObj), collectionName, 'ifo');
                        PfdKey = PfdKey.endsWith(':NDP') ? PfdKey.replace(':NDP', '') : PfdKey;
                        let subPo, subnodeid, subnodetype;
                        if (!(await this.redisService.exist(PfdKey + ':PO', process.env.CLIENTCODE)))
                            throw `${PfdKey} not found`;
                        ...
                        let apiValue = Object.values(apiConfig)[0];
                        ...
                        if (currentFabric == 'PF-SCDL') {
                            ...
                            const parentQueue = this.listenerService.getQueue(parentJobName);
                            const childQueue = this.listenerService.getQueue(childJobName);
                            ...
                            const childJob = await childQueue.add(`${childJobName}`, pfdto, {
                                parent: {
                                    id: parentJob.id,
                            ...
                            try {
                                const queueEvents = new bullmq_1.QueueEvents(childJobName, {
                                    connection: {
                            ...
                            if (!process.env.BE_URL)
                                throw new customException_1.CustomException('Server Url not found', 404);
                        ...
                        if (subPoResult?.statusCode == 201 && subPoResult?.status == 'Success') {
                            if (subPoResult?.result?.message == 'Success') {
                                ...
                                if (targetStatus != pfExecutedEvent) {
                                    throw new customException_1.CustomException('Event Mismatched in subflow', 400);
                                }
                            }
            ...
            if (nodeType == 'outputnode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    if (responseNodeName?.length == 0)
                        throw new customException_1.CustomException('outputDataNodes not found', 404);
                    for (let p = 0; p < pfjson.length; p++) {
                        if (responseNodeName.includes(pfjson[p].nodeId)) {
                            var connectedNodeName = pfjson[p].nodeName;
                        }
                    ...
                    if (inputData) {
                        ...
                        if (connectorType == 'database') {
                            ...
                            if (storageType == 'internal') {
                                ...
                                if (!specificDbType)
                                    throw new customException_1.CustomException('DB type not found', 404);
                                if (specificDbType == 'postgres') {
                                    ...
                                    if (!dbconfig || !dbconfig.MONGODB_HOST) {
                                        throw new customException_1.CustomException(`Invalid DB credentials`, 422);
                                    }
                                    let mongoDbUrl;
                                    ...
                                    client = new mongodb_1.MongoClient(mongoDbUrl);
                                    client.connect()
                                        .then(() => {
                                        console.log('Connected to the database successfully!');
                                    })
                            ...
                            if (!tableName)
                                throw new customException_1.CustomException('Table name not found', 404);
                            if (dbFlg == 'pg') {
                            ...
                            if (storageType == 'internal') {
                                ...
                                if (!redisConfig)
                                    throw new customException_1.CustomException('RedisConfig not found', 422);
                                if (!redisConfig.REDIS_HOST || !parseInt(redisConfig.REDIS_PORT)) {
                                    throw new customException_1.CustomException('Invalid Redis credentials', 400);
                                }
                                ...
                                if (!streamConfig?.host || !streamConfig?.port) {
                                    throw new customException_1.CustomException('Invalid stream credentials', 400);
                                }
                                redis = new ioredis_1.default({
                            ...
                            if (!streamName || !fieldName)
                                throw new customException_1.CustomException('streamName or fieldName not found', 404);
            ...
            if (nodeType == 'datasetschemanode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    if (!nodeVersion)
                        throw new customException_1.CustomException('Node version not found', 404);
                    if (!referenceKey)
                        throw new customException_1.CustomException('API Reference key not found', 404);
                        ...
                        if (internalEdges && internalEdges.hasOwnProperty(poNode[j].nodeId)) {
                            ...
                            for (let j = 0; j < edgesarr.length; j++) {
                                ...
                                if (targetHandle.includes('.')) {
                                    ...
                                    let staticRemove = targetVaribale.filter((item) => !this.statickeyword.includes(item));
                            ...
                            if (currentFabric == 'DF-DFD') {
                                ...
                                if (!dsSchema)
                                    throw new customException_1.CustomException('DS_Schema doesnot exist', 404);
                            ...
                            throw new customException_1.CustomException(`Data Mapping not found for ${poNode[j].nodeName}`, 404);
                        }
                    }
                }
                catch (error) {
                    ...
                    if (customConfig) {
                        ...
                        if (DfdFabric == 'DF-DFD') {
                            if (!nodeVersion)
                                throw new customException_1.CustomException('Node version not found', 404);
                            if (!DfdKey)
                                throw new customException_1.CustomException('DFD key not found', 404);
                            if (!executionMode)
                                throw new customException_1.CustomException('Execution mode not found', 404);
                            ...
                            if (executionMode == 'refresh') {
                                ...
                                if (!DfExecutedDataSet || DfExecutedDataSet.length == 0)
                                    throw new customException_1.CustomException(`Dataset not found ${DstKey + 'DS_Object'}`, 404);
                            }
                            ...
                            if (keyMergedArr.length == 0)
                                throw new customException_1.CustomException('Dataset not found', 404);
                            if (upId) {
            ...
            if (nodeType == 'api_outputnode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    if (!inputparam)
                        throw new customException_1.CustomException('Input param not found', 404);
                    let customConfig = ndp[poNode[j].nodeId];
                    let referenceKey = customConfig?.apiKey;
                    let nodeVersion = customConfig?.nodeVersion;
                    if (!nodeVersion)
                    ...
                    let edgesarr;
                    let RCMresult = await this.CommonService.getRuleCodeMapper(poNode[j], inputparam, processedKey + upId, currentFabric, SessionInfo);
                    ...
                    if (!nds)
                        throw new customException_1.CustomException('nds not found', 404);
                    let nodeid = nds[0].id;
                    ...
                    if (!methodName)
                        throw new customException_1.CustomException('Method not found', 404);
                    ...
                    if (!models) {
                        throw new customException_1.CustomException('Models not found', 404);
                    }
                    ...
                    if (internalEdges && internalEdges.hasOwnProperty(poNode[j].nodeId) && internalEdges[(poNode[j].nodeId)].length > 0) {
                        ...
                        throw new customException_1.CustomException(`Edges not found in ${poNode[j].nodeId}`, 404);
                    }
                    let dstVariable = '';
                    let dtovariable = '';
                    let sourcepath = [];
                    ...
                    if (loopingkey?.length > 0) {
                        for (let l = 0; l < loopingkey.length; l++) {
                            let targetarr = [], sourcearr = [], routearr = [];
                            for (let m = 0; m < targetpath.length; m++) {
                                if (targetpath[m].includes(loopingkey[l])) {
            ...
            if (nodeType == 'jsonparsernode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    if (oprname == "parseJson") {
                        ...
                        if (!valid) {
                            throw new customException_1.CustomException('Payload does not match schema', 400);
                        }
                        else {
            ...
            if (nodeType == 'json2xmlnode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    const xmlData = (0, xml_js_1.json2xml)(jsonString, { compact: true, spaces: 4 });
            ...
            if (nodeType == 'xml2jsonnode' && poNode[j].nodeId == nodeId) {
                try {
                    ...
                    let jsonData = await (0, xml2js_1.parseStringPromise)(parsedXml);
            ...
            if (nodeType == 'xlsx2jsonconverternode' && poNode[j].nodeId == nodeId) {
                try {
                                    ...
                                    if (connectedHandle.includes('responses')) {
                                        ...
                                        if (poNode[j].nodeType == 'xlsx2jsonconverternode') {
                                            ...
                                            if (fileType === 'csv') {
                                                ...
                                                throw new customException_1.CustomException('File type not supported', 404);
                                            }
                                        }
                                        tempArr = await this.combineData(innerpathVal, tempArr);
                                    }
                    ...
                    if (childInsertArr?.length == 0)
                        throw new customException_1.CustomException(`Mapping was required in ${poNode[j].nodeName}`, 404);
                    ...
                    if (childInsertArr?.length > 0) {
                        for (let i = 0; i < childInsertArr.length; i++) {
                            ...
                            if (mapobj && Object.keys(mapobj).length > 0) {
                                ...
                                throw new customException_1.CustomException('params was required in ' + nodeName, 400);
                            }
                        }
                    }
                    else {
    ...
    async assignToInputParam(inputparam, nodeName, data) {
        if (inputparam) {
            if (Array.isArray(inputparam) && inputparam.length > 0) {
                for (let r = 0; r < inputparam.length; r++) {
    ...
    async executeApiCall(methodName, apiUrl, requestConfig, body) {
        let apiResult;
        switch (methodName.toLowerCase()) {
            ...
            case 'delete':
                ...
                break;
            default:
                throw new customException_1.CustomException(`Unsupported HTTP method: ${methodName}`, 400);
        }
        if (apiResult.statusCode === 201 || apiResult.statusCode === 200) {
            return apiResult;
        }
    ...
    async flattenJson(data) {
        if (!Array.isArray(data)) {
            data = [data];
        }
        return data.flatMap((item) => this.flattenObject(item));
    }
    flattenObject(obj, parentKey = '', parentContext = {}) {
        const result = [];
        const flatPart = {};
        const nestedArrays = {};
            ...
            return data.map((item) => {
                ...
                for (const [targetPath, mapEntry] of Object.entries(mapping)) {
                    ...
                    if (typeof mapEntry === 'string') {
                        ...
                        const mappedArray = arrayData.map((entry) => {
                            const mappedObj = {};
    ...
    async consolidateArrayMappings(mapping) {
        const consolidated = {};
        const arrayGroups = {};
        for (const [key, value] of Object.entries(mapping)) {
            const isObjectWithArrayMap = typeof value === 'object' &&
                value !== null &&
                'sourcePath' in value &&
                'arrayMap' in value;
            ...
            if (isObjectWithArrayMap) {
                const { sourcePath, arrayMap } = value;
    ...
    removeNestedArrays(obj) {
        if (Array.isArray(obj)) {
            return obj.flat().map((item) => this.removeNestedArrays(item));
        }
        else if (typeof obj === 'object' && obj !== null) {
            return Object.fromEntries(Object.entries(obj).map(([key, value]) => [key, this.removeNestedArrays(value)]));
        }
        return obj;
    }
    processJson(data) {
    ...
    async reorderTargetPaths(edges, schema) {
        ...
        for (const path of generatedPaths) {
            ...
            const fuzzyMatchIndex = edges.targetpath.findIndex(p => p.replace(/\[0\]/g, '') === normalizedPath);
    ...
    async extractPathsFromSchema(schemaNode, currentPath, collectedPaths) {
    ...
    async APItransformData(edges, dataSets) {
        const MergedDataset = await this.mergingDataSet(dataSets);
        const mappingConfig = await this.APIcreateMappingConfig(edges, dataSets);
        ...
        const transformEntry = (entry, mapping) => {
            const transformed = {};
            for (const [targetPath, mapEntry] of Object.entries(mapping)) {
                if (typeof mapEntry === 'string') {
        ...
        const transformedData = MergedDataset.map((entry) => transformEntry(entry, mappingConfig));
        const cleanedData = await this.processJson(transformedData);
        return JSON.stringify(cleanedData, null, 2);
    }
    async APIcreateMappingConfig(edges, dataSet) {
        const mappingConfig = {};
        const arrayFields = new Set();
        ...
        function detectArrayPaths(obj, currentPath = "") {
            if (Array.isArray(obj)) {
                arrayFields.add(currentPath);
                if (obj.length > 0)
                    detectArrayPaths(obj[0], currentPath);
            }
            else if (typeof obj === "object" && obj !== null) {
                for (const key in obj) {
                    const nextPath = currentPath ? `${currentPath}.${key}` : key;
        ...
        edges.sourcepath.forEach((sourcePath, index) => {
            ...
            if (arrayIndex !== -1) {
                ...
                const isArray = [...arrayFields].some(arrayField => cleanSourcePrefix.startsWith(arrayField));
                if (!mappingConfig[arrayKey]) {
                    mappingConfig[arrayKey] = {
    ...
    async recursiveFilter(query, orderdata) {
        if (!query || !orderdata?.length)
            return orderdata;
        let nestedValue;
    ...
    extractDataWithArrayExpansion(data, targetPaths) {
        const result = {};
    ...
    async DFDMapEdgeValues(poNode, currentNodeEdge, inputparam, processedKey, upId, collectionName, parameter, codeObj, pfo, fabric) {
        var mapObj = {};
        var textobj;
        var tempQryVal = [];
        let afp = {};
        for (let s = 0; s < currentNodeEdge.length; s++) {
            ...
            for (var h = 0; h < poNode.length; h++) {
                if (connectedid == poNode[h].nodeId) {
                    var conncectedNodename = poNode[h].nodeName;
                    var conncectedNodeType = poNode[h].nodeType;
                    afp[connectedid] = JSON.parse(await this.redisService.getJsonData(processedKey +
                        upId +
                        ':NPV:' +
                        conncectedNodename +
                        '.PRO', collectionName));
        ...
        for (let e = 0; e < currentNodeEdge.length; e++) {
            ...
            if (srcHandle) {
                ...
                if (srcVal.includes('.')) {
                    ...
                    var sourceFilteredVal = staticRemove.filter((item) => !this.statickeyword.includes(item));
                    if (sourceFilteredVal && sourceFilteredVal.length > 0) {
                        sourceFilteredVal = sourceFilteredVal.join('.');
                ...
                if (srcVal.includes('requestBody') ||
                ...
                else if (srcVal.includes('responses') ||
                ...
                if (targetHandle) {
                    ...
                    if (targetVal.includes('.')) {
                        ...
                        var targetFilteredVal = staticRemove.filter((item) => !this.statickeyword.includes(item));
                        if (targetFilteredVal && targetFilteredVal.length > 0) {
                            var tempobj = {};
                            targetFilteredVal = targetFilteredVal.join('.');
                            if (targetFilteredVal.includes('.') &&
                            ...
                            if (sourceFilteredVal &&
                                sourceFilteredVal.length > 0) {
                                let checkdata = _.get(inputCollection, sourceFilteredVal);
                                ...
                                if (checkdata != null && checkdata != undefined) {
                                    ...
                                    if (codecheck != null && codecheck != undefined)
                                        _.set(mapObj, targetFilteredVal, _.get(codeObj, sourceFilteredVal));
                                }
                            }
                            else if (b == 0) {
                                let testdata = inputCollection;
                                testdata = testdata.replace(/\\n/g, '\n');
                                mapObj[targetFilteredVal] = testdata;
                            }
                        }
                        else if (sourceFilteredVal &&
                            sourceFilteredVal.length > 0) {
                            textobj = _.get(inputCollection, sourceFilteredVal);
                        }
                    }
    ...
    async mapEdgeValuesToParams(pfdto, currentNodeEdge, inputparam, processedKey, upId, collectionName, parameter, codeObj, pfo, childtable) {
        try {
            let childInsertArr = [];
            let srcIdArr = [];
            let mapObj, tempQryVal, targetVal, staticRemove, textobj;
    ...
    async getCombinations(srcIdArr, nodesArr) {
    ...
    async buildRequestComponents(apiUrl, tempQryVal, mapObj) {
        const headers = {};
        const queryParams = [];
        if (!tempQryVal || tempQryVal.length === 0) {
    ...
    async filterApiResponse(apires, filterParams) {
        if (!filterParams?.length)
            return apires;
        let filterarr = apires.filter((item) => filterParams.every(f => item[f.key] === f.value));
    ...
    async paginateResult(data, page, count) {
        if (!page || !count || !Array.isArray(data))
            return data;
    ...
    generateMockData(schema) {
        if (schema?.allOf && Array.isArray(schema.allOf)) {
            const allOfValues = schema.allOf.map((subSchema) => this.generateMockData(subSchema));
            if (allOfValues.every(val => typeof val === 'object' && !Array.isArray(val))) {
                return Object.assign({}, ...allOfValues);
            }
        }
        if (schema?.oneOf && Array.isArray(schema.oneOf)) {
            const allOfValues = schema.oneOf.map((subSchema) => this.generateMockData(subSchema));
            if (allOfValues.every(val => typeof val === 'object' && !Array.isArray(val))) {
                return Object.assign({}, ...allOfValues);
            }
        }
    ...
    findMatchingValuesFlexible(jsonData, path, expectedValue) {
    ...
    transformBySchema(schema, data) {
        ...
        switch (type) {
            ...
            case 'array': {
                ...
                return data.map((item) => this.transformBySchema(itemSchema, unwrapSingleValue(item)));
            }
            case 'string': {
...
exports.DynamicFlowService = DynamicFlowService;
exports.DynamicFlowService = DynamicFlowService = DynamicFlowService_1 = __decorate([
...
//# sourceMappingURL=dynamicFlow.service.js.map