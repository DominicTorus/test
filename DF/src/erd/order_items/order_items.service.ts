...
export class order_itemsService {
  ...
  async findSchema (token) {
    const data = {
      ...
      is_active:"boolean",
      v_no:"number",
      id :"number",      trs_creator_email:"string",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      if(quantity){ 
        query.quantity = { [queryCondition['quantity']]: quantity };
      }
      if(unit_price){ 
        query.unit_price = { [queryCondition['unit_price']]: unit_price };
      }
      if(line_total){ 
        query.line_total = { [queryCondition['line_total']]: line_total };
      }
      if(is_active){ 
      ...
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.order_items.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'order_items');
      ...
      const totalItems = await this.prismaService.order_items.count({
        where: query,
      });
  ...
  async findOne(order_id:number,token : string) {
    try{
      const res = await this.prismaService.order_items.findUnique({ 
      where: {order_id},
      select: {order_id:true,product_id:true,product_name:true,quantity:true,unit_price:true,line_total:true,is_active:true,v_no:true,id :true,        trs_creator_email:true,
    ...
    return  await this.decryptData(res, 'order_items');
  } catch (error) {
...
,order_id?:number) {
    try{
      ...
      const res = await this.prismaService.order_items.findMany({ 
      where: whereClause,
      select: {order_id:true,product_id:true,product_name:true,quantity:true,unit_price:true,line_total:true,is_active:true,v_no:true,        id :true,        trs_creator_email:true,
        trs_created_date:true,
        trs_created_by:true,
  ...
  async create(createorder_itemsDto: Prisma.order_itemsCreateInput,token:string) {
    try{
      ...
      const dataSchema:any =  v.object({
        ...
        let validate : any = v.safeParse(dataSchema,createorder_itemsDto);
        if (!validate.success) {
  ...
  async update(order_id:number, updateorder_itemsDto: Prisma.order_itemsUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateorder_itemsDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      const res = await this.prismaService.order_items.update({
      where: {order_id},
      data: await this.encryptData(updateorder_itemsDto,'order_items','update'),
      select: {order_id:true,product_id:true,product_name:true,quantity:true,unit_price:true,line_total:true,is_active:true,v_no:true,id :true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return await this.decryptData(res, 'order_items');
    } catch (error) {
  ...
  async remove(order_id:number,token : string) {
    try{
      const res = await this.prismaService.order_items.delete({
      where: {order_id },
      select: {order_id:true,product_id:true,product_name:true,quantity:true,unit_price:true,line_total:true,is_active:true,v_no:true,id :true,trs_creator_email:true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_next_status:true,trs_status:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true}
    });
    return res;
  ...
  async findFirst(token : string) {
    try{
      const res = await this.prismaService.order_items.findFirst({ 
