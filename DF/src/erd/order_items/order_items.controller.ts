...
export class order_itemsController {
  ...
  async findOne(@Headers() authHeader: string,,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.order_itemsService.findOne(,token);
  ...
  async update(@Headers() authHeader: string,
    @Body() updateorder_itemsDto: Prisma.order_itemsUpdateInput,
    @Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result = this.order_itemsService.update(,updateorder_itemsDto,token);
    return plainToInstance(order_itemsEntity, result);
  }
  ...
  async remove(@Headers() authHeader: string,,@Req() req: any) {
    const token = req.headers?.authorization?.split(' ')[1];
    //await this.ufservice.introspectToken(authHeader,"",token);
    const result =  this.order_itemsService.remove(,token);
    return plainToInstance(order_itemsEntity, result);
  }  
