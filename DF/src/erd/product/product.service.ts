...
export class productService {
  ...
  async findSchema (token) {
    const data = {
      ...
      userid :"number",      trs_created_date:"Date",
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { product_id }: {product_id : bigint} = queryValue;
      const { product_name }: {product_name : string} = queryValue;
      const { product_address }: {product_address : string} = queryValue;
      ...
      if(product_address){ 
        query.product_address = { [queryCondition['product_address']]: product_address };
      }
      const skip = (page - 1) * limit;
      if (Object.keys(query).length > 0) {
        const banks = await this.prismaService.product.findMany({
          select:columns,
          where: query,          
        });
        let decryptedRes: any = [];
        for (const indiviual of banks) {
          const decryptedData = await this.decryptData(indiviual, 'product');
  ...
  async findOne(product_id:number,token : string) {
    try{
      const res = await this.prismaService.product.findUnique({ 
      where: {product_id},
      select: {product_id:true,product_name:true,product_address:true,userid :true,        trs_created_date:true,
        trs_created_by:true,
    ...
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'product');
  } catch (error) {
  ...
  async findAll(token : string,trs_created_date?: Date,trs_created_by?: string,trs_modified_date?: Date,trs_modified_by?: string,trs_process_id?: string,trs_access_profile?: string,trs_org_grp_code?: string,trs_org_code?: string,trs_role_grp_code?: string,trs_role_code?: string,trs_ps_grp_code?: string,trs_ps_code?: string,trs_sub_org_grp_code?: string,trs_sub_org_code?: string,trs_locked_by?: string,trs_locked_time?: Date,trs_tenant_id?:string,trs_app_code?:string,trs_product_code?:string,trs_event_process_status?:string,trs_event_status?:string) {
    try{
      ...
      where: whereClause,
      select: {product_id:true,product_name:true,product_address:true,          userid :true,    
      ...
      for (const indiviual of res) {
        ...
        const decryptedData = await this.decryptData(plain, 'product');
        decryptedRes.push(decryptedData);
      }
      return decryptedRes;
    } catch (error) {
  ...
  async create(createproductDto: Prisma.productCreateInput,token:string) {
    try{

      const dataSchema:any =  v.object({
            product_name :  v.optional(v.string()), 
            product_address :  v.optional(v.string()), 
        });
        let validate : any = v.safeParse(dataSchema,createproductDto);
        if (!validate.success) {
  ...
  async createMaster(
    ...
    try {
      ...
      if (role === 'CHECKER') {
        ...
        if (approvalStatus === 'APPROVED') {
          ...
          if (success) {
            return {
              success: true,
              message: 'product creation approved and applied successfully',
          ...
          if (success) {
            return {
              success: true,
              message: 'product creation rejected',
              approval_id: approvalId,
              status: 'REJECTED'
            };
          } else {
      ...
      const encryptedData = await this.encryptData(createproductDto, 'product', 'create');
  ...
  async update(product_id:number, updateproductDto: Prisma.productUpdateInput,token:string) {   
    try{
      ...
      let validate : any = v.safeParse(dataSchema,updateproductDto);
      if (!validate.success) {
        let errorObj: errorObj = {
          tname: 'TG',
          errGrp: 'Data',
      ...
      where: {product_id},
      data: await this.encryptData(updateproductDto,'product','update'),
      select: {product_id:true,product_name:true,product_address:true,userid :true,trs_created_date:true,trs_created_by:true,trs_modified_date:true,trs_modified_by:true,trs_process_id:true,trs_access_profile:true,trs_org_grp_code:true,trs_org_code:true,trs_role_grp_code:true,trs_role_code:true,trs_ps_grp_code:true,trs_ps_code:true,trs_sub_org_code:true,trs_sub_org_grp_code:true,trs_locked_by:true,trs_locked_time:true,trs_tenant_id:true,trs_app_code:true,trs_product_code:true,trs_event_process_status:true,trs_event_status:true}
    });
    return await this.decryptData(await this.commonDecimalDatahandle(res), 'product');
    } catch (error) {
...
product_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const updateMaster_id =product_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'product update approved and applied successfully',
      ...
      const existingRecord = await this.prismaService.product.findUnique({
        where: {product_id}
      });
      ...
      // Encrypt data if needed
      const encryptedData = await this.encryptData(updateproductDto, 'product', 'update');
  ...
  async remove(product_id:number,token : string) {
    try{
...
product_id:number,
    ...
    try {
      const role = userInfo.role?.toUpperCase();
      const deleteMaster_id =product_id;
      ...
      if (role === 'CHECKER') {
        ...
        if (success) {
          return {
            success: true,
            message: 'product deletion approved and applied successfully',
            approval_id: approvalId,
