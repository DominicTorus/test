...
export class userService {
 ...
 async findAllmethod(queryDto: any, limit:number,selectColumns:any,token:any) {
    try {
      ...
      let query: any = {}; 
      const { user_id }: {user_id : number} = queryValue;
      const { name }: {name : string} = queryValue;
      const { email }: {email : string} = queryValue;
      const { designation }: {designation : string} = queryValue;
      ...
      await this.commonService.errorLog(
        ...
        "TG020",
        error,
        "CK:CT003:FNGK:AF:FNK:API-ERD:CATK:RD001:AFGK:RDS001:AFK:Sample_ERD:AFVK:v1",
        token
      );
  ...
  async findOne(user_id:number,token : string) {
    try{
...
,user_id?:number) {
  ...
  async update(user_id:number, updateuserDto: Prisma.userUpdateInput,token:string) {   
    try{
...
user_id:number,
  ...
  async remove(user_id:number,token : string) {
    try{
