...
const Textnamedf = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if(Array.isArray(dfd_carddoc_v1Props) && dfd_carddoc_v1Props){
        setdynamic9403b((pre:any)=>({...pre,t_name:dfd_carddoc_v1Props[0]?.t_name}));
      }
    }catch(err){
      console.log(err)
    }
  ...
  if (namedf0caad?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 6`,gridRow: `4 / 13`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Text 
      className=""
      variant ="body-3"
      color ="primary"
       ellipsis={true}
    >
    {isDynamic ? item?.t_name : (dynamic9403b?.t_name || "")}
    </Text> 
  </div>
  )
}

export default Textnamedf
