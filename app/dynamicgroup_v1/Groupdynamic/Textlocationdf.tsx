...
const Textlocationdf = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if(Array.isArray(dfd_carddoc_v1Props) && dfd_carddoc_v1Props){
        setdynamic9403b((pre:any)=>({...pre,t_location:dfd_carddoc_v1Props[0]?.t_location}));
      }
    }catch(err){
      console.log(err)
    }
  ...
  useEffect(()=>{
    handleMapperValue()
  },[locationdf8b23d?.refresh])

  if (locationdf8b23d?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 9`,gridRow: `42 / 51`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Text 
      className=""
      variant ="body-3"
      color ="primary"
    ...
    <span className="flex items-center gap-3">
      <Icon size= {16} data= {MapPin}   />
      {isDynamic ? item?.t_location : (dynamic9403b?.t_location || "")}
    </span>
    </Text> 
  </div>
  )
