...
const Textplandf = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if(Array.isArray(dfd_carddoc_v1Props) && dfd_carddoc_v1Props){
        setdynamic9403b((pre:any)=>({...pre,t_plan:dfd_carddoc_v1Props[0]?.t_plan}));
      }
    }catch(err){
      console.log(err)
    }
  ...
  if (plandf36f3e?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `9 / 13`,gridRow: `4 / 13`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Text 
      className=""
      variant ="body-3"
      color ="primary"
       ellipsis={true}
    >
    {isDynamic ? item?.t_plan : (dynamic9403b?.t_plan || "")}
    </Text> 
  </div>
  )
}

export default Textplandf
