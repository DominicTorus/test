...
import {PersonsLock} from '@gravity-ui/icons';
...
const Textemployeedf = ({encryptionFlagCompData,isDynamic,item,index}:any) => {
  ...
  const handleMapperValue=async()=>{
    try{
      if(Array.isArray(dfd_carddoc_v1Props) && dfd_carddoc_v1Props){
        setdynamic9403b((pre:any)=>({...pre,t_employees:dfd_carddoc_v1Props[0]?.t_employees}));
      }
    }catch(err){
      console.log(err)
    }
  ...
  useEffect(()=>{
    handleMapperValue()
  },[employeedfb0755?.refresh])

  if (employeedfb0755?.isHidden) {
    return <></>
  }

return (
  <div className="" style={{gridColumn: `2 / 9`,gridRow: `30 / 39`, gap:``, height: `100%`, overflow: 'auto'}} >
    <Text 
      className=""
      variant ="body-3"
      color ="primary"
    ...
    <span className="flex items-center gap-3">
      <Icon size= {16} data= {PersonsLock}   />
      {isDynamic ? item?.t_employees : (dynamic9403b?.t_employees || "")}
    </span>
    </Text> 
  </div>
  )
