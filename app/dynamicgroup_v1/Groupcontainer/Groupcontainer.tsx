...
import { Magnifier,Xmark } from '@gravity-ui/icons'
  import Groupdynamic  from "../Groupdynamic/Groupdynamic";
...
const Groupcontainer = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,isFormOpen=false}:any) => {
 ...
 /////////////
  ...
  /////////////
    if(securityData[accessProfile]?.['readOnlyControls'].includes("dynamic")){
      setdynamic9403b({...dynamic9403b,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const container72d6dRef = useRef<any>(null);
  const handleClearSearch = () => {
    container72d6dRef.current?.setSearchParams();
    container72d6dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(container72d6d) && Object.keys(container72d6d)?.length>0)
      {
        setcontainer72d6d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [container72d6dProps?.refresh])
  ...
  <div  
      ...
      className="border border-gray-400 shadow-sm rounded-md "
      >
        {securityData[accessProfile]?.allowedGroups?.includes("dynamic")  &&<Groupdynamic  
