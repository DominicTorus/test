...
import Groupcgroup  from "../Groupcgroup/Groupcgroup";
...
const Groupcompanygroup = ({lockedData={},setLockedData,primaryTableData={}, setPrimaryTableData,checkToAdd,setCheckToAdd,refetch,setRefetch,dropdownData,setDropdownData,encryptionFlagPageData, nodeData, setNodeData,paginationDetails,isFormOpen=false}:any)=> {
 ...
 /////////////
  ...
  const orchestrationData:any = await AxiosService.post("/UF/Orchestration",{key:"CK:CT003:FNGK:AF:FNK:UF-UFW:CATK:CG:AFGK:TG1:AFK:Company:AFVK:v1",componentId:"c261eda188f3426bbca420ea97e72d6d",from:"GroupCompanygroup",accessProfile:accessProfile},{
    headers: {
  ...
  /////////////
    if(orchestrationData?.data?.readableControls.includes("cgroup")){
      setcgroupf48bf({...cgroupf48bf,isDisabled:true});
    }
  //////////////
    if (code != '') {
      let codeStates: any = {};
  ...
  const companygroup72d6dRef = useRef<any>(null);
  const handleClearSearch = () => {
    companygroup72d6dRef.current?.setSearchParams();
    companygroup72d6dRef.current?.handleSearch({});
  };
  ...
  useEffect(() => {    
    ...
    if (prevRefreshRef.current) {
      if(!Array.isArray(companygroup72d6d) && Object.keys(companygroup72d6d)?.length>0)
      {
        setcompanygroup72d6d({})
      }
    }else 
      prevRefreshRef.current= true
  }, [companygroup72d6dProps?.refresh])
    ...
    <div 
      style={{
       ...
       className={`${getBorderRadiusClass(branding.borderRadius)} ${direction === 'RTL' ? 'rtl' : 'ltr'}`}
                        dir={direction}
    >
        {allowedComponent.includes("cgroup")  &&<Groupcgroup  
...
export default Groupcompanygroup
