...
const TextInputtextInut = ({checkToAdd,setCheckToAdd,refetch,setRefetch,encryptionFlagCompData}:any) => {  
  ...
  let schemaArray :any =[];  
  const [dynamicStateandType,setDynamicStateandType]=useState<any>({name:'textinut',type:"text"})
  ...
  const handleChange = async(e: any) => {
    setError('')
    setValidate((pre:any)=>({...pre,textinut:undefined}))
    if(dynamicStateandType.type=="number"){
    setgroup2911c((prev: any) => ({ ...prev, textinut: +e.target.value }))
    }
    else{
    setgroup2911c((prev: any) => ({ ...prev, textinut: e.target.value }))
    }
    ...
    try{
      const orchestrationData: any = await AxiosService.post(
        '/UF/Orchestration',
        {
          ...
          controlId: "ddfccc9b0abd4278820dfd6a7b8187a8",
          isTable: false,
          from:"TextInputtextinut",
      ...
      if(orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties){
        let type:any={name:'textinut',type:'text'}
        type={
          name:'textinut',
          type: orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.textinut.type == 'string' ? 'text' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.textinut.type =='integer' ? 'number' : orchestrationData?.data?.schemaData[0].schema.responses["200"].content["application/json"].schema.items.properties.textinut.type
        }
      ...
      //  if(Object.keys(orchestrationData?.data?.dstData).length>0) 
       // setgroup2911c((pre:any)=>({...pre,textinut:orchestrationData?.data?.dstData}))
      }
    }
    catch(err)
    {
  ...
  if (textinut187a8?.isHidden) {
    return <></>
  }
  return (   
    <div 
      style={{gridColumn: `2 / 4`,gridRow: `6 / 13`, gap:``, height: `100%`, overflow: 'auto'}} >
      ...
      <TorusTextInput
        ...
        value={group2911c?.textinut||""}
         disabled= {textinut187a8?.isDisabled ? true : false}
        pin='brick-brick'     
        placeholder='type here....'      
        readOnly= {textinut187a8?.isDisabled ? true : false}
        size='m'      
        view='normal'
        validationState={validate?.textinut ? "invalid" : undefined}
        errorMessage={error}
      />
    </div> 
  )
...
export default TextInputtextInut
